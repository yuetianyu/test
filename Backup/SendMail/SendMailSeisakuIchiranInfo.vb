...
Imports EBom.Common
Imports EBom.Data
Imports SendMail.SendMail.Dao


Module subSendMailSeisakuIchiranInfo
    ...
    Sub Main()
        ...
        Try
            ...
            'ユーザーＩＤ情報
            Dim UserIdList As New List(Of MailAddressVo)
            Dim MailList As New List(Of MailAddressVo)
            UserIdList = getUserIdList()

            If StringUtil.Equals(UserIdList, Nothing) = False Then
                'メールアドレス取得
                MailList = getMailAddressList(UserIdList)
...
#Region " メールアドレス取得（ＡＤ） "
    ...
    Private Function getMailAddressList(ByVal userIdList As List(Of MailAddressVo)) As List(Of MailAddressVo)

        '初期設定
        Dim i As Integer

        'Dim objDSEItem          ' rootDSE内容保持オブジェクト
        'Dim objFso, objDomain
...
#Region "ユーザーＩＤ取得"
    Private Function getUserIdList() As List(Of MailAddressVo)
