...
Namespace Ui.Valid.Impl
    ...
    '''どちらでもないなら混在
    ''' </remarks>
    Friend Class HankakuValidator(Of T) : Inherits AbstractValidator(Of T)

        Private Const DEFAULT_MESSAGE As String = "{0} は、半角文字を入力して下さい."
        ...
        Protected Overrides Function ValidateImpl(ByVal index As Integer) As Boolean
            ...
            '文字列長とバイト長が同じ場合には半角のみと判定できる
            If byteLength = buf.ToString.Length Then
                Return True
            End If
