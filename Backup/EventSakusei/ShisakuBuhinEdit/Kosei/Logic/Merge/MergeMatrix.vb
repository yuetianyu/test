...
Namespace ShisakuBuhinEdit.Kosei.Logic.Merge
    ...
    Friend Class MergeMatrix
        ...
        Private isIkanshaKaishu As Boolean
        ...
        ''' <remarks></remarks>
        Public Sub New(ByVal matrix As BuhinKoseiMatrix, Optional ByVal isIkanshaKaishu As Boolean = False)
            Me._matrix = matrix
...
#End Region
        ...
        ''' <remarks></remarks>
        Friend Sub Compute(ByVal columnIndex As Integer, ByVal aData As IData, Optional ByVal KaiteiNo As String = "")
            ...
            If Not IsFirstColumnIndex(columnIndex) Then
                ...
                For Each rowIndex As Integer In RecordIndexes
                    ...
                    If CanMerge(Record(rowIndex), aData) _
                       AndAlso (aData.GetLevel Is Nothing OrElse aData.GetLevel = 0 OrElse (0 < aData.GetLevel)) _
                       And Not (Record(rowIndex).Level = 0 And aData.GetLevel = 0) Then
                        '↑↑2014/09/30 酒井 ADD END
                        ...
                        If InsuVo(rowIndex, columnIndex).BuhinNoHyoujiJun <> aData.GetInsuVo.BuhinNoHyoujiJun _
                           AndAlso InsuVo(rowIndex, columnIndex).InstlHinbanHyoujiJun = aData.GetInsuVo.InstlHinbanHyoujiJun _
                           AndAlso InsuVo(rowIndex, columnIndex).ShisakuBlockNo = aData.GetInsuVo.ShisakuBlockNo _
                           AndAlso InsuVo(rowIndex, columnIndex).ShisakuBlockNoKaiteiNo = aData.GetInsuVo.ShisakuBlockNoKaiteiNo _
                           AndAlso InsuVo(rowIndex, columnIndex).ShisakuBukaCode = aData.GetInsuVo.ShisakuBukaCode _
                           AndAlso InsuVo(rowIndex, columnIndex).ShisakuEventCode = aData.GetInsuVo.ShisakuEventCode _
                           AndAlso InsuVo(rowIndex, columnIndex).InstlHinbanHyoujiJun IsNot Nothing _
                           AndAlso InsuVo(rowIndex, columnIndex).ShisakuBlockNo IsNot Nothing _
                           AndAlso InsuVo(rowIndex, columnIndex).ShisakuBlockNoKaiteiNo IsNot Nothing _
                           AndAlso InsuVo(rowIndex, columnIndex).ShisakuBukaCode IsNot Nothing _
                           AndAlso InsuVo(rowIndex, columnIndex).ShisakuEventCode IsNot Nothing Then
            ...
            If KaiteiNo = "" Or aData.GetLevel = 0 Then
        ...
        Friend Interface IData
            ...
            ''' <remarks></remarks>
            Function GetSaishiyouFuka() As String
            ''' <summary>
            ''' 供給セクションを返す
            ''' </summary>
            ...
            '2014/09/23 酒井 ADD
            Function GetBaseBuhinFlg() As String

            Function GetBaseBuhinSeq() As Nullable(Of Int32)

            Function GetInstlDataKbn() As String
        ...
        Private Function CanMerge(ByVal recordRecordVo As BuhinKoseiRecordVo, ByVal aData As IData) As Boolean
            ...
            If Me.isIkanshaKaishu Then
                Return EzUtil.IsEqualIfNull(recordRecordVo.Level, aData.GetLevel) _
                        ...
                        AndAlso EzUtil.IsEqualIfNull(recordRecordVo.BuhinNoKbn, aData.GetBuhinNoKbn) _
                        AndAlso EzUtil.IsEqualIfNull(recordRecordVo.Saishiyoufuka, aData.GetSaishiyouFuka) _
                        AndAlso EzUtil.IsEqualIfNull(recordRecordVo.BaseBuhinFlg, aData.GetBaseBuhinFlg) _
                        AndAlso EzUtil.IsEqualIfNull(recordRecordVo.KyoukuSection, aData.GetKyoukuSection) _
                        AndAlso EzUtil.IsEqualIfNull(recordRecordVo.BaseBuhinSeq, aData.GetBaseBuhinSeq)
            End If
            ...
            '2014/09/23 酒井 ADD
            'AndAlso EzUtil.IsEqualIfNull(recordRecordVo.BaseBuhinFlg, aData.GetBaseBuhinFlg) _を追加
