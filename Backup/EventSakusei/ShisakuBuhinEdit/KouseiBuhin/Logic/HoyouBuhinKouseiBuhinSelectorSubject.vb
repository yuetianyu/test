...
Imports EventSakusei.XVLView.Dao.Vo         '   Add 2014.04.15 by S.Tokizaki
Imports EventSakusei.XVLView.Dao.Impl       '   Add 2014.04.15 by S.Tokizaki
...
Namespace KouseiBuhin.Logic
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Class HoyouBuhinKouseiBuhinSelectorSubject
...
#Region "メンバー変数"
        ...
        Private _frmKouseiBuhinSelector As HoyouBuhinFrm41KouseiBuhinSelector
...
#Region "部品SPREADカラム"
        ...
        ''' 
        '''     Originalでは列追加の度に値を振りなおしがあるので、
        '''     今回の修正で、少々小細工した。
        '''     本来ならば、Enumにしたい。
        ''' 
        '''     Changed 2014.04.15 by S.Tokizaki
        ''' 
#If 1 Then
...
#Region "コンストラクタ"
        ...
        ''' <remarks></remarks>
        Public Sub New(ByVal aHoyouEventCode As String, ByVal frm As HoyouBuhinFrm41KouseiBuhinSelector)
...
#Region "ヘッダー部初期化"
        ...
        Public Sub InitializeHeader()
            ...
            Dim frm As HoyouBuhinFrm41KouseiBuhinSelector = _frmKouseiBuhinSelector
            ...
            'イベントコードとイベント名の設定
            'frm.lblEventCode.Text = _HoyouEventCode
            'frm.lblEventName.Text = _HoyouEventName
...
#Region "開発符号のコンボボックスを作成"
        ...
        ''' </summary>
        ''' <remarks></remarks>
        Public Sub SetKaihatsuFugoCombo(Optional ByVal shisakuEventCode As String = "")
            ...
            Try
                ...
                For idx As Integer = 0 To dtKaihatsufugo.Rows.Count - 1
                    ...
                    vos.Add(vo)
                    '↓↓2014/10/09 酒井 ADD BEGIN
                    If eventVo.ShisakuKaihatsuFugo = vo.Value Then
                        flg = True
                    End If
                    '↑↑2014/10/09 酒井 ADD END
                Next
                ...
                '↓↓2014/10/09 酒井 ADD BEGIN
                frm.cmbKaihatsuFugo.SelectedValue = eventVo.ShisakuKaihatsuFugo
                '↑↑2014/10/09 酒井 ADD END
            Catch ex As Exception
        ...
        End Function
        '↓↓2014/10/09 酒井 ADD BEGIN
        Public Sub SetInitialView(ByVal shisakuEventCode As String)

            '先頭のベース号車の開発符号
            Dim impl As KouseiBuhinSelectorDao = New KouseiBuhinSelectorDaoImpl
            Dim eventBaseVo As TShisakuEventBaseVo = impl.GetInitialKaihatsuFugo(shisakuEventCode)
            _frmKouseiBuhinSelector.cmbKaihatsuFugo.SelectedValue = eventBaseVo.BaseKaihatsuFugo
            ...
            Try
                ...
                Else
                    ...
                    If strLevel = "0" AndAlso frm.spdBuhin_Sheet1.Cells(nNextRow, spd_Buhin_Col_Level).Value = "0" Then
                        ...
                        Dim buhinstruct As New HoyouBuhinTehaichoEditBuhinStructure(_HoyouEventCode, "ListCodeDummy", strBlockNo, "BukaCodeDummy", Shisakudate)
                        Dim newBuhinMatrix As HoyouBuhinBuhinKoseiMatrix = buhinstruct.GetKouseiMatrix(strBuhinNo, "", 0, strKaihatuFugo)
                        ...
                        For Each i As Integer In newBuhinMatrix.GetInputRowIndexes
                            ...
                            'レベル０の行は無条件で部品リストに追加
                            If StringUtil.Equals(newBuhinMatrix(i).Level, 0) Then
...
#End If
                ...
                '3D画面表示処理
                Dim iXVlWindow As New HoyouBuhinFrmVeiwerImge
        ...
        Private Function Get3DFileList(ByVal aKaihatuFugo As String, ByVal aBlockNo As String, ByVal aBuhinList As List(Of String)) As List(Of String)
            ...
            For Each iBuhin In aBuhinList
                Dim iVo As New List(Of XVLView.Dao.Vo.RHAC2270Vo)
                Dim iArg As New XVLView.Dao.Vo.RHAC2270Vo()
...
#Region "ブロック名称のコンボボックスを作成"
        ...
        ''' </summary>
        ''' <remarks></remarks>
        Public Sub SetBlockNameCombo()
            Try
                'ブロック名称コンボボックスに値を追加
                Dim dtBlockName As DataTable = GetBlockNameData()
                Dim vos As New List(Of LabelValueVo)
        ...
        ''' <returns>全データテーブル</returns>
        ''' <remarks></remarks>
        Private Function GetBlockNameData() As DataTable
            ...
            Using db As New SqlAccess(g_kanrihyoIni(DB_KEY_mBOM))
                db.Open()
                db.Fill(DataSqlCommon.GetBlockNameSql(frm.cmbKaihatsuFugo.Text), dtData)
            End Using
            Return dtData
        End Function
...
#Region "フォームから呼出し仕様情報一覧のコンボボックスを生成"
        ...
        '''' <summary>仕向地・仕向けのコンボボックスを生成する。</summary>
        '''' <remarks></remarks>
        'Public Sub setShiyouJyouhouListShimukechiShimuke()

        '    Dim frm As HoyouBuhinFrm41KouseiBuhinSelector = _frmKouseiBuhinSelector
        '    SpreadUtil.BindCellTypeToColumn(frm.spdBuhinShiyou_Sheet1, _
        '                                    DispShiyouJyouhouList.TAG_SHIMUKECHI_SHIMUKE.ToString, _
        '                                    NewShimukechiShimukeCellType)
        '    'カウント
        '    Dim nCount As Integer = 0
        '    For i As Integer = 0 To NewShimukechiShimukeCellType.Items.Length - 1
        '        nCount += 1
        '    Next
        '    'ITEM数が１なら画面に表示する。
        '    If StringUtil.Equals(nCount, 1) Then
        '        frm.spdBuhinShiyou_Sheet1.SetValue(spd_BuhinShiyou_startRow, _
        '        frm.spdBuhinShiyou_Sheet1.Columns(DispShiyouJyouhouList.TAG_SHIMUKECHI_SHIMUKE.ToString).Index(), _
        '        NewShimukechiShimukeCellType.Items(0).ToString())
        '    Else
        '        frm.spdBuhinShiyou_Sheet1.SetValue(spd_BuhinShiyou_startRow, _
        '            frm.spdBuhinShiyou_Sheet1.Columns(DispShiyouJyouhouList.TAG_SHIMUKECHI_SHIMUKE.ToString).Index(), "")
        '    End If
        ...
        Public Sub setKataOpListShimukechiHandle()
            ...
            '    NewKataOpCellType.Items(0).ToString())
            'Else
            '    frm.spdBuhinShiyou_Sheet1.SetValue(spd_BuhinShiyou_startRow, _
...
#Region "OP項目列の作成"
        ...
        Public Sub setOpKoumokuRetuHandle()
            ...
            If OpListVos.Count > 0 Then
                ...
                'If frm.spdBuhinShiyou_Sheet1.ColumnCount > 15 Then
                '    frm.spdBuhinShiyou_Sheet1.RemoveColumns(15, frm.spdBuhinShiyou_Sheet1.ColumnCount - 15)
                'End If
                ''今回取得したOP項目数分列を挿入する。
                'frm.spdBuhinShiyou_Sheet1.AddColumns(14, OpListVos.Count - 2)
                ''ＯＰ項目列に値を設定する。
                'For i As Integer = 13 To 13 + OpListVos.Count - 1
                ...
                '    'セルタイプを変更
                '    frm.spdBuhinShiyou_Sheet1.Cells(spd_BuhinShiyou_startRow, i).CellType = NewComboBoxCellType
                '    '
                '    'ブランクを設定
                '    frm.spdBuhinShiyou_Sheet1.Cells(spd_BuhinShiyou_startRow, i).Text = ""
                '    j += 1
                'Next
                'frm.spdBuhinShiyou_Sheet1.Cells(0, 13).ColumnSpan = OpListVos.Count
                'frm.spdBuhinShiyou_Sheet1.Cells(0, 13, spd_BuhinShiyou_OpSpecNameRow, 13 + OpListVos.Count - 1).HorizontalAlignment = FarPoint.Win.Spread.CellHorizontalAlignment.Center
                'frm.spdBuhinShiyou_Sheet1.Cells(0, 13, spd_BuhinShiyou_OpSpecNameRow, 13 + OpListVos.Count - 1).VerticalAlignment = FarPoint.Win.Spread.CellVerticalAlignment.Top
                'frm.spdBuhinShiyou_Sheet1.Cells(0, 13, spd_BuhinShiyou_OpSpecNameRow, 13 + OpListVos.Count - 1).Font = New Font("MS UI Gothic", 8, FontStyle.Regular)
                'frm.spdBuhinShiyou_Sheet1.Cells(0, 13, spd_BuhinShiyou_OpSpecNameRow, 13 + OpListVos.Count - 1).BackColor = Color.Yellow
                'frm.spdBuhinShiyou_Sheet1.Cells(1, 13, spd_BuhinShiyou_OpSpecNameRow, 13 + OpListVos.Count - 1).CellType = NewTextType
                ''ＯＰ項目列をロックする。
                'frm.spdBuhinShiyou_Sheet1.Cells(1, 13, spd_BuhinShiyou_OpSpecNameRow, 13 + OpListVos.Count - 1).Locked = True
                ''幅を調整（コンボボックスなので４０にしている）
                'frm.spdBuhinShiyou_Sheet1.Columns(13, 13 + OpListVos.Count - 1).Width = 40
...
#Region "開発符号、ブロック、仕様情報に該当するファイナル品番を返す。"
        ...
        ''' <returns>表示値</returns>
        ''' <remarks></remarks>
        Public Function GetFinalHinbanValues(ByVal KaihatsuFugo As String, ByVal BlockNo As String, ByVal finalBuhin As String) As List(Of FinalHinbanListVo)
            ...
            Return ShiyouJyouhouDao.GetByFinalHinbanValues(KaihatsuFugo, BlockNo, finalBuhin, _
                                                           frm.spdBuhinShiyou_Sheet1.GetValue(spd_BuhinShiyou_startRow, _
                                                           frm.spdBuhinShiyou_Sheet1.Columns(DispShiyouJyouhouList.TAG_SHIYOU_JYOUHOU_NO.ToString).Index()), _
                                                           frm.spdBuhinShiyou_Sheet1.GetValue(spd_BuhinShiyou_startRow, _
                                                           frm.spdBuhinShiyou_Sheet1.Columns(DispShiyouJyouhouList.TAG_SYAGATA.ToString).Index()), _
...
#Region "検索イベント"
        ...
        ''' <remarks></remarks>
        Public Sub doSearchEvent(ByVal kaihatsuFugo As String, ByVal selectKbn As String)
            'Try
            ...
            If StringUtil.Equals(frm.cmbSelect.Text, HOYOU_SELECT_MBOM_SHISAKUTEHAI) Then
                ...
                frm.pnlFinal.Visible = False 'Final指定検索用
                frm.txtFinal.Enabled = False
                frm.btnFinal.Enabled = False
                ''↑↑2014/07/29 Ⅰ.3.設計編集 ベース車改修専用化_ax) (TES)張 ADD END
                '
                ShiyouFormSeigyo("MBOM")
                ''↓↓2014/07/29 Ⅰ.3.設計編集 ベース車改修専用化_ax) (TES)張 ADD BEGIN
            ElseIf StringUtil.Equals(frm.cmbSelect.Text, HOYOU_SELECT_FINAL) Then
                ...
                frm.btnShiyouOnOff.Visible = False
                frm.btnReset.Visible = False
                frm.pnlFinal.Visible = True 'Final指定検索用
                frm.txtFinal.Enabled = True
                frm.btnFinal.Enabled = True
                ShiyouFormSeigyo("MBOM")
                ''↑↑2014/07/29 Ⅰ.3.設計編集 ベース車改修専用化_ax) (TES)張 ADD END
            Else
                frm.cmbShisakuEventCode.Enabled = False
            ...
            Select Case selectKbn
                Case HOYOU_SELECT_EBOM_SYSTEM_DAI
                    ''開発符号でシステム大区分を取得
                    'Dim systemDaiKbnList As List(Of SystemKbnVo) = kouseiBuhinDao.GetBySystemDaiKbnDataEbom(kaihatsuFugo)
                    '
                    ...
                    ''開発符号でブロックを取得
                    'Dim blockList As List(Of BlockListVo) = kouseiBuhinDao.GetByBlockDataEbom(kaihatsuFugo)
                    '
                    frm.spdBlock_Sheet1.RowCount = 0

                    Try
                        m_blockList = New DispBlockList(frm)
                        m_blockList.InitView(kaihatsuFugo, "", "")
                    Catch ex As Exception
                        Throw
                    End Try
                    '
                ...
                Case HOYOU_SELECT_EBOM_SYSTEM
                    ''開発符号でシステム区分を取得
                    'Dim systemKbnList As List(Of SystemKbnVo) = kouseiBuhinDao.GetBySystemKbnDataEbom(kaihatsuFugo)
                    '
                    frm.spdKubun_Sheet1.RowCount = 0
        ...
        Public Sub doSearchGousya(ByVal strKaihatsuFugo As String, _
                                  ...
                                  ByVal strTantoKey As String, ByVal strTanto As String)
            Try
                ...
                '試作イベント名称で号車を取得
                'Dim gousyaList As List(Of GousyaListVo) = kouseiBuhinDao.GetByGousyaDataMbomFromC(strKaihatsuFugo, strShisakuEvent)
                '
                frm.spdKubun_Sheet1.RowCount = 0
                Try
                ...
                '号車でブロックを取得
                'Dim blockList As List(Of BlockListVo) = kouseiBuhinDao.GetByBlockDataMbomFromC(strKaihatsuFugo, strShisakuEvent)
                '
                frm.spdBlock_Sheet1.RowCount = 0
                Try
                    m_blockList = New DispBlockList(frm)
                    m_blockList.InitView(strKaihatsuFugo, strShisakuEvent, strShisakuEventName)
                Catch ex As Exception
                    Throw
                End Try
...
#Region "部品番号表示"
        ...
        Public Sub ExecuteShisakuBuhinInfo(ByVal kaihatsuFugo As String, _
            ...
            Try
                ...
                Dim kouseiBuhinDao As KouseiBuhinSelectorDao = New KouseiBuhinSelectorDaoImpl
                'Dim buhinList As New List(Of TShisakuBuhinEditVo)
                Dim buhinList As New List(Of TehaichoMenu.Vo.TShisakuBuhinEditVoSekkeiHelper)

                If StringUtil.IsEmpty(shisakuEventName) Then
                    buhinList = kouseiBuhinDao.GetByShisakuBuhinToSpreadFromC(kaihatsuFugo, shisakuEventCode, blockListVos)
                Else
                    buhinList = kouseiBuhinDao.GetByShisakuBuhinToSpreadFromN(kaihatsuFugo, shisakuEventName, blockListVos)
                End If
                ...
                'For Each bVo As TShisakuBuhinEditVo In buhinList
                For Each bVo As TehaichoMenu.Vo.TShisakuBuhinEditVoSekkeiHelper In buhinList
                    ...
                    '自給品有無＝無の場合　国内または国外集計コードＪは読み飛ばす。
                    If jikyu = "1" Or _
                        jikyu = "0" And bVo.ShukeiCode.ToString <> "" And bVo.ShukeiCode.ToString <> "J" Or _
                        jikyu = "0" And bVo.ShukeiCode.ToString = "" And bVo.SiaShukeiCode.ToString <> "J" Then
                        ...
                        frm.spdBuhin_Sheet1.Cells(i, spd_Buhin_Col_ShukeiCode).Value = bVo.ShukeiCode.ToString
                        frm.spdBuhin_Sheet1.Cells(i, spd_Buhin_Col_SiaShukeiCode).Value = bVo.SiaShukeiCode.ToString
                        '構成表示ボタン
                        ...
                        'frm.spdBuhin_Sheet1.Cells(i, spd_Buhin_Col_Insu).Value = 1
...
#Region "E-BOMから部品構成情報取込処理＆チェックを付ける処理"
        ...
        Public Function ExecuteBuhinKouseiInfo(ByVal aBlockNo As String, _
                                               ...
                                               ByVal nRow As Integer, Optional ByVal btnHaneiFlg As Boolean = False) As Boolean
        ...
        Public Function ExecuteBuhinKouseiCheck(ByVal aBlockNo As String, _
            ...
            Try
                ...
                'If frm.spdBuhin_Sheet1.Cells(nRow, 1).Value <> 0 Then
                '    'If frm.spdBuhin_Sheet1.Cells(nRow, 5).Value = True Then
                '    '    frm.spdBuhin_Sheet1.Cells(nRow, 5).Value = False
                '    'Else
                '    '    frm.spdBuhin_Sheet1.Cells(nRow, 5).Value = True
                '    'End If
        ...
        Public Function ExecuteBuhinKouseiCheckShisaku(ByVal aBlockNo As String, _
            ...
            Try
                ...
                '    If (nRow + 1) > (frm.spdBuhin_Sheet1.RowCount - 1) Then
                '        '
                '        frm.spdBuhin_Sheet1.Cells(nRow, spd_Buhin_Col_SelectionMethod).Value = HOYOU_NOMAL_ALL_SHISAKU
                '    Else
                '        '次の行が０レベルなら親品番構成全部対象。
                '        If frm.spdBuhin_Sheet1.Cells(nRow + 1, spd_Buhin_Col_Level).Value = "0" Then
                '            '
                ...
                If frm.rbtNomal.Checked = True Then
                    ...
                    '            frm.spdBuhin_Sheet1.Cells(i, spd_Buhin_Col_Select).Value = True
                    '            '構成状況。
                    '            frm.spdBuhin_Sheet1.Cells(i, spd_Buhin_Col_SelectionMethod).Value = HOYOU_NOMAL_SHISAKU
                    '            '構成状況
                    '            frm.spdBuhin_Sheet1.Cells(nRow, spd_Buhin_Col_SelectionMethod).Value = HOYOU_NOMAL_SHISAKU
                    '        Else
                    '            'チェックを外す。
                    '            frm.spdBuhin_Sheet1.Cells(i, spd_Buhin_Col_Select).Value = False
                    '            '構成状況。
                ...
                Else
                    ...
                    '                frm.spdBuhin_Sheet1.Cells(i, spd_Buhin_Col_Select).Value = True
                    '                '構成状況。
                    '                frm.spdBuhin_Sheet1.Cells(i, spd_Buhin_Col_SelectionMethod).Value = HOYOU_TANPIN_SHISAKU
                    '                '構成状況
        ...
        Public Sub ShiyouFormSeigyo(ByVal fEbomOrMbom As String)
            ...
            If StringUtil.Equals(fKakuSyuku, "KAKUDAI") Then
                ...
                If 0 < (frm.spdBuhin.Location.Y - (rSize + 35)) Then
                    '
                    'frm.spdBuhinShiyou.Size = New Size(745, 30)
                    ...
                    '
                    'frm.lblBuhinBango.Location = New Point(368, 173)
                    frm.lblBuhinBango.Location = New Point(frm.lblBuhinBango.Location.X, frm.spdBuhinShiyou.Size.Height + 143)
                    '
                    'frm.pbBuhinBango.Location = New Point(488, 169)
                    frm.pbBuhinBango.Location = New Point(frm.pbBuhinBango.Location.X, frm.spdBuhinShiyou.Size.Height + 138)
                    '
                    'frm.btnTenkai.Location = New Point(1039, 168)

                    '2014024 展開ボタンと反映ボタンの位置入れ替えに伴う対応
                    ''frm.btnTenkai.Location = New Point(frm.btnTenkai.Location.X, frm.spdBuhinShiyou.Size.Height + 138)
                    frm.BtnHanei.Location = New Point(frm.BtnHanei.Location.X, frm.spdBuhinShiyou.Size.Height + 138)


                    'frm.spdBuhin.Location = New Point(363, 187)
                    'frm.spdBuhin.Size = New Size(745, 446)
            ...
            Else
                ...
                If frm.Size.Height > (frm.spdBuhin.Location.Y + (rSize + 35)) Then
                    '
                    'frm.spdBuhinShiyou.Size = New Size(745, 245)
                    ...
                    '
                    'frm.lblBuhinBango.Location = New Point(368, 388)
                    frm.lblBuhinBango.Location = New Point(frm.lblBuhinBango.Location.X, frm.spdBuhinShiyou.Size.Height + 143)
                    '
                    'frm.pbBuhinBango.Location = New Point(488, 384)
                    frm.pbBuhinBango.Location = New Point(frm.pbBuhinBango.Location.X, frm.spdBuhinShiyou.Size.Height + 138)
                    '
                    'frm.btnTenkai.Location = New Point(1039, 383)

                    '2014024 展開ボタンと反映ボタンの位置入れ替えに伴う対応
                    ''frm.btnTenkai.Location = New Point(frm.btnTenkai.Location.X, frm.spdBuhinShiyou.Size.Height + 138)
                    frm.BtnHanei.Location = New Point(frm.BtnHanei.Location.X, frm.spdBuhinShiyou.Size.Height + 138)
                    '
                    'frm.spdBuhin.Location = New Point(363, 403)
                    'frm.spdBuhin.Size = New Size(745, 230)
                    frm.spdBuhin.Location = New Point(frm.spdBuhin.Location.X, frm.spdBuhinShiyou.Size.Height + 158)
                    frm.spdBuhin.Size = New Size(frm.spdBuhin.Size.Width, frm.Size.Height - frm.spdBuhin.Location.Y - 30)
                    '
...
#Region "部品情報取込処理"
        ...
        ''' </summary>
        ''' <remarks></remarks>
        Public Function ExecuteBuhinInfo() As Boolean
            ...
            Try
                ...
                Dim aUserId As String = LoginInfo.Now.UserId
                Dim aKaihatsuFugo As String = frm.cmbKaihatsuFugo.Text
                'Dim aImportKbn As Integer = frm.cmbImportKbn.SelectedIndex
                Dim bTenkaiInstl As Boolean = frm.rbtNomal.Checked
                '自給品の有無
                Dim aJikyu As String = ""
                If StringUtil.Equals(frm.cmbJikyuhinUmu.Text, "無") Then
                    aJikyu = "0"
                ...
                'ファイル作成(KA)
                aFileName = aUserId & aKaihatsuFugo & "KA.txt"
                Dim sw As New System.IO.StreamWriter(aPath & aFileName, False, System.Text.Encoding.GetEncoding("shift_jis"))
                sw.Write("")
                sw.Close()
                'ファイル作成(BLK)
                aFileName = aUserId & aKaihatsuFugo & "BLK.txt"
                sw = New System.IO.StreamWriter(aPath & aFileName, False, System.Text.Encoding.GetEncoding("shift_jis"))
                Dim sb As New StringBuilder

                '
                Dim selection As FarPoint.Win.Spread.Model.CellRange() = frm.spdBlock_Sheet1.GetSelections()
                For rowindex As Integer = 0 To selection.Length - 1
                    sb.AppendLine(frm.spdBlock_Sheet1.Cells(selection(rowindex).Row, 0).Value)
                Next
                ...
                '=== パラメータ ===
                'イベント情報取得
                Dim aEventId As String = GetEventID(aKaihatsuFugo)

                '=== コマンド実行 ===
                ExecuteCommand(aUserId, aKaihatsuFugo, aEventId)

                '=== データ取込 ===
                IsReturn = setData(aKaihatsuFugo, bTenkaiInstl, aJikyu)
        ...
        ''' </summary>
        ''' <remarks></remarks>
        Public Function ExecuteBuhinFinalInfo(Optional ByVal shisakuBlockNo As String = "") As Boolean
            '↓↓2014/10/09 酒井 ADD BEGIN
            ...
            Try
                ...
                ''↓↓2014/09/01 Ⅰ.3.設計編集 ベース車改修専用化_az) 酒井 ADD BEGIN
                Dim tmp As Integer
                Dim selection As FarPoint.Win.Spread.Model.CellRange() = Nothing
                '↓↓2014/10/09 酒井 ADD BEGIN
                If Not shisakuBlockNo = "" Then
                    tmp = 0
                ElseIf frm.txtFinal.Text = "" Then
                    'If frm.txtFinal.Text = "" Then
                    '↑↑2014/10/09 酒井 ADD END
                    selection = frm.spdBlock_Sheet1.GetSelections()
                    tmp = selection.Length - 1
                Else
                    tmp = 0
                End If
                ...
                'Dim selection As FarPoint.Win.Spread.Model.CellRange() = frm.spdBlock_Sheet1.GetSelections()
                'For rowindex As Integer = 0 To selection.Length - 1
                For rowindex As Integer = 0 To tmp
                    ''↑↑2014/09/01 Ⅰ.3.設計編集 ベース車改修専用化_az) 酒井 ADD END
                    ...
                    'FinalHinbanListVos = _
                    'GetFinalHinbanValues(aKaihatsuFugo, frm.spdBlock_Sheet1.Cells(selection(rowindex).Row, 0).Value, frm.txtFinal.Text)
                    '↓↓2014/10/09 酒井 ADD BEGIN
                    If Not shisakuBlockNo = "" Then
                        FinalHinbanListVos = _
                        GetFinalHinbanValues(aKaihatsuFugo, shisakuBlockNo, "")
                        'If frm.txtFinal.Text = "" Then
                    ElseIf frm.txtFinal.Text = "" Then
                        '↑↑2014/10/09 酒井 ADD END
                        FinalHinbanListVos = _
                    ...
                    Else
                        FinalHinbanListVos = _
                        GetFinalHinbanValues(aKaihatsuFugo, "", frm.txtFinal.Text)
                    End If
                    ''↑↑2014/09/01 Ⅰ.3.設計編集 ベース車改修専用化_az) 酒井 ADD END
                    ''↑↑2014/07/31 Ⅰ.3.設計編集 ベース車改修専用化_ba) (TES)張 CHG END
                    If FinalHinbanListVos.Count > 0 Then
                        ...
                        For i As Integer = 0 To FinalHinbanListVos.Count - 1
                            ...
                            ' ''--- 552から取得 ---
                            'Dim aRhac0552 As List(Of Rhac0552Vo) = ShiyouJyouhouDao.GetByRHAC0552Values(FinalHinban)
                            ''--- 553から取得 ---
                            ...
                            If StringUtil.IsNotEmpty(aRhac0553) Then
                                If aRhac0553.Count > 0 Then
                                    ...
                                    ''員数が－１なら**に置き換える。
                                    'If StringUtil.Equals(aRhac0553.Item(0).InsuSuryo.ToString, "-1") Then
                                    '    aInsu = "**"
                                    'Else
                                    '    aInsu = aRhac0553.Item(0).InsuSuryo.ToString
                                    'End If
                                    'Else
                                    'If aRhac0552.Count > 0 Then
                                    '    aShukeiCode = aRhac0552.Item(0).ShukeiCode
                                    '    aSiaShukeiCode = aRhac0552.Item(0).SiaShukeiCode
                                    '    '員数が－１なら**に置き換える。
                                    '    If StringUtil.Equals(aRhac0552.Item(0).InsuSuryo.ToString, "-1") Then
                                    '        aInsu = "**"
                                    '    Else
                                    '        aInsu = aRhac0552.Item(0).InsuSuryo.ToString
                                    '    End If
                                    'End If
                                End If
                            End If
...
#Region "イベントID取得"
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Private Function GetEventID(ByVal KaihatsuFugo As String) As String
            Dim aEventId As String = ""
            Try
                If StringUtil.IsNotEmpty(KaihatsuFugo) Then
                    Dim Rhac1320Dao As Rhac1320EventDao = New Rhac1320EventDaoImpl
                    Dim Rhac1320Vos As New List(Of Rhac1320EventVo)
                    Rhac1320Vos = Rhac1320Dao.GetByEventInfo(KaihatsuFugo)
                    aEventId = StringUtil.ToString(Rhac1320Vos(0).EventId)
                End If
            Catch ex As Exception
                MessageBox.Show(ex.Message, "異常", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End Try
...
#Region "コマンド実行"
        ...
        ''' <remarks></remarks>
        Private Sub ExecuteCommand(ByVal UserId As String, ByVal KaihatsuFugo As String, ByVal EventId As String)
            ...
            Dim sb As New StringBuilder
            sb.Append("/c ")
            sb.Append("cd /e E:\EXE")
            '列型式ベースの場合こちら
            'sb.Append(" && extPlDev2.exe " & UserId & " " & KaihatsuFugo & " 1 d " & aDate & " " & EventId & " COMP 1")
            sb.Append(" && extPlDev2.exe " & UserId & " " & KaihatsuFugo & " 1 d " & aDate & " " & EventId & " AUTO 0")
            ''列INSTLベースの場合こちら
            'sb.Append(" && extPlVari.exe " & UserId & " " & KaihatsuFugo & " 1 " & aDate & " " & EventId & " 000 NOCOMP")
...
#Region "データをセット"
        ...
        ''' </summary>
        ''' <remarks></remarks>
        Public Function setData(ByVal aKaihatsuFugo As String, ByVal bTenkaiInstl As Boolean, ByVal aJikyu As String) As Boolean
            ...
            Try
                ...
                '--- データ件数 ---
                Dim aMaxLineCnt As Integer = GetLinesOfTextFile(aPath & aFileName, bTenkaiInstl, aJikyu, "", "")
                frm.pbBuhinBango.Maximum = aMaxLineCnt + 1 '１はヘッダー部の行数
                ...
                buttonType.Text = "+"
                buttonType.TextDown = "-"
                buttonType.TextAlign = FarPoint.Win.ButtonTextAlign.TextTopPictBottom
                ...
                While Not txtParser.EndOfData
                    ...
                    Else
                        ...
                        '0レベルのみ
                        If splittedResult(columnLEVEL).ToString() = "0" Then

                            ''ALL：全部品セット、INSTL：0レベルのみセット
                            'If StringUtil.Equals(frm.rbtTenkaiAll.Checked, True) Or _
                            '   StringUtil.Equals(frm.rbtTenkaiInstl.Checked, True) And splittedResult(columnLEVEL).ToString() = "0" Then

                            If splittedResult(columnLEVEL).ToString() >= "a" And splittedResult(columnLEVEL).ToString() <= "z" Then
                            Else
                                ...
                                If aJikyu = "1" Or _
                                    ...
                                    'ブロック№
                                    frm.spdBuhin_Sheet1.Cells(intRowCnt, spd_Buhin_Col_BlockNo).Value = aAddData(basicBlockNo)
                                    'LV
                                    frm.spdBuhin_Sheet1.Cells(intRowCnt, spd_Buhin_Col_Level).Value = aAddData(basicLevel)
                                    '国内集計コード
                                    frm.spdBuhin_Sheet1.Cells(intRowCnt, spd_Buhin_Col_ShukeiCode).Value = aAddData(basicShukeiCode)
                                    '海外集計コード
                                    frm.spdBuhin_Sheet1.Cells(intRowCnt, spd_Buhin_Col_SiaShukeiCode).Value = aAddData(basicSiaShukeiCode)
                                    '構成表示ボタン
                                    frm.spdBuhin_Sheet1.Cells(intRowCnt, spd_Buhin_Col_Tenkai).CellType = buttonType
                                    frm.spdBuhin_Sheet1.Cells(intRowCnt, spd_Buhin_Col_Tenkai).BackColor = Color.LightBlue
                                    'If aAddData(basicLevel) = 0 Then
                                    ...
                                    'End If
                                    '選択チェックボックス
                                    frm.spdBuhin_Sheet1.Cells(intRowCnt, spd_Buhin_Col_Select).Value = False
                                    '部品番号レベル毎に頭に半角空文字セット
                                    frm.spdBuhin_Sheet1.Cells(intRowCnt, spd_Buhin_Col_BuhinNo).Value = Left(strYohaku, CInt(aAddData(basicLevel))) + _
                                                                                    aAddData(basicBuhinNo)
                                    '員数
                                    frm.spdBuhin_Sheet1.Cells(intRowCnt, spd_Buhin_Col_Insu).Value = strInsu
                                    '部品名称
                                    frm.spdBuhin_Sheet1.Cells(intRowCnt, spd_Buhin_Col_BuhinName).Value = aAddData(basicBuhinName)
                                    '部品ノート
                                    frm.spdBuhin_Sheet1.Cells(intRowCnt, spd_Buhin_Col_Note).Value = aAddData(basicBuhinNote)
                                    '部品構成
                                    frm.spdBuhin_Sheet1.Cells(intRowCnt, spd_Buhin_Col_SelectionMethod).Value = ""
                                    'frm.spdBuhin_Sheet1.Cells(intRowCnt, 10).Value = aAddData(basicBuhinKousei)

                                    intRowCnt = intRowCnt + 1
                                    '進度バー
                                    frm.pbBuhinBango.Value = intRowCnt
                ...
                ''****** データ登録 ******
                'If IsRegisterMain(aDtGencho, aDtGosya, aKaihatsuFugo) Then
                IsReturn = True
                'End If
            Catch ex As Exception
                Console.WriteLine("Error:" & ex.Message)
...
#Region "構成部品情報を取得"
        ...
        Public Function getKouseiData(ByVal aKaihatsuFugo As String, _
            ...
            Try
                ...
                Dim Shisakudate As ShisakuDate = New ShisakuDate
                Dim buhinstruct As New HoyouBuhinTehaichoEditBuhinStructure(_HoyouEventCode, "ListCodeDummy", aBlockNo, "BukaCodeDummy", Shisakudate)
                Dim newBuhinMatrix As HoyouBuhinBuhinKoseiMatrix = buhinstruct.GetKouseiMatrix(aInstlHinban, "", 0, aKaihatsuFugo)
                ...
                For Each i As Integer In newBuhinMatrix.GetInputRowIndexes
                    ...
                    '自分は除く
                    If Not StringUtil.Equals(newBuhinMatrix(i).Level, 0) Then
                        '値を確保
                        Dim aShukeiCode As String = newBuhinMatrix(i).ShukeiCode.TrimEnd
                        Dim aSiaShukeiCode As String = newBuhinMatrix(i).SiaShukeiCode.TrimEnd
                        Dim aLevel As String = newBuhinMatrix(i).Level
                        Dim aBuhinNo As String = newBuhinMatrix(i).BuhinNo.TrimEnd
                        Dim aBuhinName As String = newBuhinMatrix(i).BuhinName.TrimEnd
                        Dim aBuhinNote As String = newBuhinMatrix(i).BuhinNote.TrimEnd
                        Dim aKyoukuSection As String = newBuhinMatrix(i).KyoukuSection.TrimEnd
...
#End If
                            ...
                            '部品ノート
                            frm.spdBuhin_Sheet1.Cells(intRowCnt, spd_Buhin_Col_Note).Value = aBuhinNote
                            ''部品構成
                ...
                While Not txtParser.EndOfData
                    ...
                    Else
                        ...
                        If aJikyu = "1" Or _
                            ...
                            Else
                                'instl品番を選択したか？
                                'If StringUtil.Equals(bTenkaiInstl, True) Then
...
#Region "データセット(基本情報)"
        ...
        Private Function setLineOfDataBasic(ByVal wKaihatsuFugo As String, ByVal wLineCnt As Integer, ByVal wDataRow As DataRow, ByVal strTemp As String()) As DataRow
            ...
            Try
                ...
                'Dim aVo As New GenchoEditVo
                '--- 開発シリーズ取得 ---
                'aVo = getKaihatsuFugoAndHojyoName(aBuhinNo, aVo)
                'Dim aKaihatsuSerise As String = ""
                'If Not StringUtil.IsEmpty(aVo.KaihatsuSerise) Then
                '    aKaihatsuSerise = aVo.KaihatsuSerise
                'End If
                ...
                ''--- 仕様書№取得 ---
                'Dim aShiyoshoSeqno As String = getShiyoshoNo(wKaihatsuFugo)
