...
Namespace KouseiBuhin.Dao
    Public Class KouseiBuhinSelectorDaoImpl : Implements KouseiBuhinSelectorDao
        ...
        '''' <returns></returns>
        '''' <remarks></remarks>
        'Function GetByBlockToBuhinEbom(ByVal kaihatsuFugo As String, ByVal selectBlock As List(Of BlockListVo)) As List(Of BlockListVo) Implements KouseiBuhinSelectorDao.GetByBlockToBuhinEbom
        '    Dim sb As New StringBuilder
        '    With sb
        '        .Remove(0, .Length)
        '        .AppendLine("SELECT ")
...
#End Region
        ...
        Function GetByShisakuBuhinToSpreadFromC(ByVal kaihatsuFugo As String, _
            ...
            '    .Remove(0, .Length)
            '    '.AppendLine("SELECT * ")
            '    .AppendLine("SELECT EDIT.*, BEI.INSU_SURYO ")
            ...
            '        '.AppendLine(",").Append("'").Append(BlockVo.BlockNo).Append("'")
        ...
        Function GetInitialKaihatsuFugo(ByVal shisakuEventCode As String) As TShisakuEventBaseVo Implements KouseiBuhinSelectorDao.GetInitialKaihatsuFugo
            Dim sb As New StringBuilder
            With sb
                .Remove(0, .Length)
                .AppendLine("SELECT * ")
                .AppendLine("FROM " & MBOM_DB_NAME & ".dbo.T_SHISAKU_EVENT_BASE WITH (NOLOCK, NOWAIT) ")
                .AppendLine("WHERE SHISAKU_EVENT_CODE = '" & shisakuEventCode & "' ")
                .AppendLine("AND HYOJIJUN_NO= ")
                .AppendLine(" ( ")
                .AppendLine("   SELECT MIN(CONVERT(INT,COALESCE(HYOJIJUN_NO,''))) AS HYOJIJUN_NO  ")
                .AppendLine("   FROM " & MBOM_DB_NAME & ".dbo.T_SHISAKU_EVENT_BASE WITH (NOLOCK, NOWAIT) ")
                .AppendLine("   WHERE SHISAKU_EVENT_CODE = '" & shisakuEventCode & "' ")
                .AppendLine("   ) ")
            End With
            ...
            Dim db As New EBomDbClient
            Return db.QueryForObject(Of TShisakuEventBaseVo)(sb.ToString)
        End Function
