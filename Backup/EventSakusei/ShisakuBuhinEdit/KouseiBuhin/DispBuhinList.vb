...
#Region " メンバー変数 "
        ...
        Private Const TAG_3D As String = "3D"           '   3D有無の表示列のタグを追加　S.Tokizaki
...
#Region " SPREADの列のタグ値を設定する "
        ...
        Public Sub SetSpdColTag()
            ...
            '   ここの、宣言はセル位置を示す
            '
            Dim _KouseiBuhinSelectorLogic As New Logic.HoyouBuhinKouseiBuhinSelectorSubject("", m_view)


            '
            '   列タグを設定するに当たり、即値をやめた
            '   S.Tokizaki 2014.05.14
            '
...
#Region "SPREADで 列のセルの水平方向の配置を設定する。"
        Public Sub SetSpdColPro()
            ...
            'For i As Integer = 0 To m_view.spdBuhin_Sheet1.ColumnCount - 1
            '    m_view.spdBuhin_Sheet1.Columns(i).Resizable = False
            'Next
...
#Region " SPREADの列の値を設定する "
        ''' <summary>
        ''' SPREADの値を設定する        ''' </summary>
        ''' <remarks></remarks>
        Public Shared Sub SetSpdData(ByVal aAddData As DataRow)

            'm_view.spdBuhin_Sheet1.Cells(intRowNo, m_spCom.GetColFromTag(TAG_BLOCK_NO).Index).Value = aAddData(1)
