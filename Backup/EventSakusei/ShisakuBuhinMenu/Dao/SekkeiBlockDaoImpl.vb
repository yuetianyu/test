...
Namespace ShisakuBuhinMenu.Dao
    Public Class SekkeiBlockDaoImpl : Implements SekkeiBlockDao
        ...
        ''' <remarks></remarks>
        Public Function FindAlByShisakuEventBaseIroAri(ByVal shisakuEventCode As String, Optional ByVal HyojiJunNo As String = "") As List(Of SekkeiBlockAlResultVo) Implements SekkeiBlockDao.FindAlByShisakuEventBase
            ...
            With Sql
                ...
                .AppendLine("     INNER JOIN " & MBOM_DB_NAME & ".dbo.T_SHISAKU_EVENT_BASE BASE ")
        ...
        ''' <returns>該当した「A/Lの素情報」</returns>
        ''' <remarks></remarks>
        Public Function FindAlByShisakuEventBaseIroAriNoBuhin(ByVal shisakuEventCode As String, Optional ByVal HyojiJunNo As String = "") As List(Of SekkeiBlockAlResultVo) Implements SekkeiBlockDao.FindAlByShisakuEventBaseNoBuhin
            Dim sql As New StringBuilder()
            '★指定された仕様情報№のブロックを抽出。
            With sql
                .Remove(0, .Length)
                .AppendLine(" SELECT AL.BLOCK_NO, ")
                '/*** 20140911 CHANGE START（未使用のため） ***/
                '.AppendLine(" CASE WHEN MAX(AL2.FUKA_NO) IS NULL AND MAX(COLORM.FUKA_NO) IS NULL THEN AL.F_BUHIN_NO ")
                '.AppendLine("      WHEN MAX(AL2.FUKA_NO) IS NOT NULL AND MAX(COLORM.FUKA_NO) IS NULL THEN LEFT(AL.F_BUHIN_NO,10)+ MAX(AL2.FUKA_NO) ")
                '.AppendLine("      WHEN MAX(AL2.FUKA_NO) IS NULL AND MAX(COLORM.FUKA_NO) IS NOT NULL THEN LEFT(AL.F_BUHIN_NO,10)+ MAX(COLORM.FUKA_NO) ")
                '.AppendLine("      WHEN MAX(AL2.FUKA_NO) IS NOT NULL AND MAX(COLORM.FUKA_NO) IS NOT NULL THEN LEFT(AL.F_BUHIN_NO,10)+MAX( AL2.FUKA_NO) ")
                ...
                .AppendLine(" WHERE BASE.SHISAKU_EVENT_CODE = @ShisakuEventCode AND ")
                If HyojiJunNo <> "" Then
                    .AppendLine(" BASE.HYOJIJUN_NO = @HyojiJunNo AND ")
                End If
                '/*** 20140911 CHANGE END ***/
                .AppendLine("     BLOCKM.TANTO_BUSHO IS NOT NULL AND ")
                .AppendLine("     AL.SHIYOSA_KAITEI_NO = (SELECT MAX(SHIYOSA_KAITEI_NO) FROM " & RHACLIBF_DB_NAME & ".dbo.RHAC2210 WITH (NOLOCK, NOWAIT, INDEX(PK_RHAC2210))")
            ...
            '/*** 20140911 CHANGE START（パラメータ化） ***/
            'Return db.QueryForList(Of SekkeiBlockAlResultVo)(sql.ToString())
            Dim param As New SekkeiBlockAlConditionVo(shisakuEventCode, HyojiJunNo, Nothing)
        ...
        ''' <returns>該当した「A/Lの素情報」</returns>
        ''' <remarks></remarks>
        Public Function FindAlByShisakuEventBaseIroAriByBlock(ByVal shisakuEventCode As String, ByVal blockNo As String) As List(Of SekkeiBlockAlResultVo) Implements SekkeiBlockDao.FindAlByShisakuEventBaseByBlock
            Dim sql As New StringBuilder()
            '★指定された仕様情報№のブロックを抽出。
            With Sql
                .Remove(0, .Length)
                ...
                '.AppendLine(" BLOCKM.TANTO_BUSHO AS BUKA_CODE, ")
                .AppendLine(" TMP.SHISAKU_BUKA_CODE AS BUKA_CODE, ")
                ...
                .AppendLine(" ON TMP.SHISAKU_EVENT_CODE = @ShisakuEventCode")
                .AppendLine(" AND TMP.SHISAKU_BLOCK_NO = AL.BLOCK_NO")
                ...
                If StringUtil.IsNotEmpty(blockNo) Then
                    .AppendLine(" AL.BLOCK_NO = @BlockNo AND ")
                End If
                '/*** 20140911 CHANGE END ***/
                .AppendLine("     BLOCKM.TANTO_BUSHO IS NOT NULL AND ")
                .AppendLine("     AL.SHIYOSA_KAITEI_NO = (SELECT MAX(SHIYOSA_KAITEI_NO) FROM " & RHACLIBF_DB_NAME & ".dbo.RHAC2210 WITH (NOLOCK, NOWAIT, INDEX(PK_RHAC2210))")
            ...
            '/*** 20140911 CHANGE START（パラメータ化） ***/
            'Return db.QueryForList(Of SekkeiBlockAlResultVo)(sql.ToString())
            Dim param As New SekkeiBlockAlConditionVo(shisakuEventCode, blockNo)
            Return db.QueryForList(Of SekkeiBlockAlResultVo)(sql.ToString(), param)
            '/*** 20140911 CHANGE END ***/
        ...
        ''' <returns>該当した「A/Lの素情報」</returns>
        ''' <remarks></remarks>
        Public Function FindAlByShisakuEventBaseIroAri2(ByVal shisakuEventCode As String, Optional ByVal HyojiJunNo As String = "") As List(Of SekkeiBlockAlResultVo) Implements SekkeiBlockDao.FindAlByShisakuEventBase2
            Dim sql As New StringBuilder()
            '★指定された仕様情報№のブロックを抽出。
            With sql
                .Remove(0, .Length)
        ...
        ''' <returns>該当した「A/Lの素情報」</returns>
        ''' <remarks></remarks>
        Public Function FindAlByShisakuEventBaseIroAri2NoBuhin(ByVal shisakuEventCode As String, Optional ByVal HyojiJunNo As String = "") As List(Of SekkeiBlockAlResultVo) Implements SekkeiBlockDao.FindAlByShisakuEventBase2NoBuhin
            Dim sql As New StringBuilder()
            '★指定された仕様情報№のブロックを抽出。
            With sql
                .Remove(0, .Length)
        ...
        ''' <returns>該当した「A/Lの素情報」</returns>
        ''' <remarks></remarks>
        Public Function FindAlByShisakuEventBaseIroAri2ByBlock(ByVal shisakuEventCode As String, ByVal BlockNo As String) As List(Of SekkeiBlockAlResultVo) Implements SekkeiBlockDao.FindAlByShisakuEventBase2ByBlock
            Dim sql As New StringBuilder()
            '★指定された仕様情報№のブロックを抽出。
            With sql
                .Remove(0, .Length)
        ...
        ''' <returns>該当した「A/Lの素情報」</returns>
        ''' <remarks></remarks>
        Public Function FindAlByShisakuEventBaseIroAriOLD(ByVal shisakuEventCode As String, Optional ByVal HyojiJunNo As String = "") As List(Of SekkeiBlockAlResultVo) Implements SekkeiBlockDao.FindAlByShisakuEventBaseOLD
            Dim sql As New StringBuilder()
        ...
        ''' <returns>該当した「A/Lの素情報」</returns>
        ''' <remarks></remarks>
        Public Function FindAlByShisakuEventBaseIroAriOLDNoBuhin(ByVal shisakuEventCode As String, Optional ByVal HyojiJunNo As String = "") As List(Of SekkeiBlockAlResultVo) Implements SekkeiBlockDao.FindAlByShisakuEventBaseOLDNoBuhin
            Dim sql As New StringBuilder()
            '★指定された仕様情報№の-1のブロックを抽出。
            With sql
                .Remove(0, .Length)
        ...
        ''' <returns>該当した「A/Lの素情報」</returns>
        ''' <remarks></remarks>
        Public Function FindAlByShisakuEventBaseIroAriOLDByBlock(ByVal shisakuEventCode As String, ByVal blockNo As String) As List(Of SekkeiBlockAlResultVo) Implements SekkeiBlockDao.FindAlByShisakuEventBaseOLDByBlock
            Dim sql As New StringBuilder()
            '★指定された仕様情報№の-1のブロックを抽出。
            With sql
                .Remove(0, .Length)
        ...
        ''' <returns>該当した「A/Lの素情報」</returns>
        ''' <remarks></remarks>
        Public Function FindAlByShisakuEventBaseIroAriOLD2(ByVal shisakuEventCode As String, Optional ByVal HyojiJunNo As String = "") As List(Of SekkeiBlockAlResultVo) Implements SekkeiBlockDao.FindAlByShisakuEventBaseOLD2
            Dim sql As New StringBuilder()
            '★指定された仕様情報№の-1のブロックを抽出。
            With sql
                .Remove(0, .Length)
        ...
        ''' <returns>該当した「A/Lの素情報」</returns>
        ''' <remarks></remarks>
        Public Function FindAlByShisakuEventBaseIroAriOLD2NoBuhin(ByVal shisakuEventCode As String, Optional ByVal HyojiJunNo As String = "") As List(Of SekkeiBlockAlResultVo) Implements SekkeiBlockDao.FindAlByShisakuEventBaseOLD2NoBuhin
            Dim sql As New StringBuilder()
            '★指定された仕様情報№の-1のブロックを抽出。
            With sql
                .Remove(0, .Length)
        ...
        ''' <returns>該当した「A/Lの素情報」</returns>
        ''' <remarks></remarks>
        Public Function FindAlByShisakuEventBaseIroAriOLD2ByBlock(ByVal shisakuEventCode As String, ByVal blockNo As String) As List(Of SekkeiBlockAlResultVo) Implements SekkeiBlockDao.FindAlByShisakuEventBaseOLD2ByBlock
            Dim sql As New StringBuilder()
            '★指定された仕様情報№の-1のブロックを抽出。
            With sql
                .Remove(0, .Length)
        ...
        '''' <returns>該当した「A/Lの素情報」</returns>
        '''' <remarks></remarks>
        'Public Function FindAlByShisakuEventBaseIroNashi(ByVal shisakuEventCode As String) As List(Of SekkeiBlockAlResultVo) Implements SekkeiBlockDao.FindAlByShisakuEventBaseIroNashi

        '    Dim sql As String = ""

        '    '色コード無の情報を抽出する。★まずは指定された仕様情報№のブロックを抽出。
        '    sql = _
        '    " SELECT AL.BLOCK_NO, " _
        '    & " AL.F_BUHIN_NO AS FF_BUHIN_NO, " _
        '    & " AL.F_BUHIN_NO AS BF_BUHIN_NO, " _
        '    & " KA.BU_CODE, " _
        '    & " KA.KA_CODE, " _
        '    & " BASE.SHISAKU_GOUSYA, " _
        '    & " BLOCKM.TANTO_BUSHO AS BUKA_CODE, " _
        '    & " BLOCK_NAME, " _
        '    & " AL.SHIYOSA_KAITEI_NO , " _
        '    & " AL.SOBI_KAITEI_NO " _
        '    & " FROM " & RHACLIBF_DB_NAME & ".dbo.RHAC2210 AL WITH (NOLOCK, NOWAIT) " _
        '    & "     INNER JOIN " & MBOM_DB_NAME & ".dbo.T_SHISAKU_EVENT_BASE BASE " _
        '    & "         ON AL.KAIHATSU_FUGO = BASE.BASE_KAIHATSU_FUGO " _
        '    & "             And AL.KATASHIKI_SCD_7 = BASE.BASE_KATASHIKI_SCD_7 " _
        '    & "             And AL.SHIMUKECHI_CODE = BASE.BASE_SHIMUKE " _
        '    & "             And AL.OP_CODE = BASE.BASE_OP " _
        '    & "             AND AL.F_BUHIN_NO <>  '         0     ' " _
        '    & "             And AL.SOBI_KAITEI_NO = BASE.BASE_SOBI_KAITEI_NO " _
        '    & "             AND LEFT(AL.BLOCK_NO,3) <>  '999' " _
        '    & "     LEFT OUTER JOIN " _
        '    & "         " & RHACLIBF_DB_NAME & ".dbo.RHAC0080 BLOCKM ON " _
        '    & "             BLOCKM.BLOCK_NO_KINO = AL.BLOCK_NO " _
        '    & "             And BLOCKM.KAIHATSU_FUGO = AL.KAIHATSU_FUGO " _
        '    & "             AND LEFT(BLOCKM. BLOCK_NO_KINO, 3) <> '999' " _
        '    & "             And BLOCKM.HAISI_DATE = 99999999 " _
        '    & "     LEFT OUTER JOIN " _
        '    & "         " & RHACLIBF_DB_NAME & ".dbo.RHAC0040 BLOCKNAME ON " _
        '    & "             BLOCKNAME.BLOCK_NO = AL.BLOCK_NO " _
        '    & "     LEFT JOIN " & RHACLIBF_DB_NAME & ".dbo.RHAC1560 KA " _
        '    & "         ON BLOCKM.TANTO_BUSHO = KA.KA_RYAKU_NAME " _
        '    & "             ,(SELECT '0' AS DIV UNION ALL SELECT '1') C " _
        '    & " WHERE BASE.SHISAKU_EVENT_CODE = @Value AND " _
        '    & "     RIGHT(AL.F_BUHIN_NO,2) <> '##' AND " _
        '    & "     BLOCKM.TANTO_BUSHO IS NOT NULL AND " _
        '    & "     AL.SHIYOSA_KAITEI_NO = (SELECT MAX(SHIYOSA_KAITEI_NO) FROM " & RHACLIBF_DB_NAME & ".dbo.RHAC2210 " _
        '    & "     WHERE(KAIHATSU_FUGO = AL.KAIHATSU_FUGO) " _
        '    & "     And KATASHIKI_SCD_7 = AL.KATASHIKI_SCD_7 " _
        '    & "     And COL_NO = AL.COL_NO " _
        '    & "     And BLOCK_NO = AL.BLOCK_NO " _
        '    & "     AND SOBI_KAITEI_NO = AL.SOBI_KAITEI_NO ) " _
        '    & " GROUP BY " _
        '    & "     AL.BLOCK_NO, " _
        '    & "     AL.F_BUHIN_NO, " _
        '    & "     KA.BU_CODE, " _
        '    & "     KA.KA_CODE, " _
        '    & "     BASE.SHISAKU_GOUSYA, " _
        '    & "     BLOCKM.TANTO_BUSHO, " _
        '    & "     BLOCK_NAME, " _
        '    & "     AL.SHIYOSA_KAITEI_NO, " _
        '    & "     AL.SOBI_KAITEI_NO "

        '    Dim db As New EBomDbClient
        '    Return db.QueryForList(Of SekkeiBlockAlResultVo)(sql, shisakuEventCode)
        'End Function
        ...
        '''' <returns>該当した「A/Lの素情報」</returns>
        '''' <remarks></remarks>
        'Public Function FindAlByShisakuEventBaseIroNashiOLD(ByVal shisakuEventCode As String) As List(Of SekkeiBlockAlResultVo) Implements SekkeiBlockDao.FindAlByShisakuEventBaseIroNashiOLD

        '    Dim sql As String = ""

        '    '色コード無の情報を抽出する。★指定された仕様情報№の-1のブロックを抽出。
        '    sql = _
        '    " SELECT AL.BLOCK_NO, " _
        '    & " AL.F_BUHIN_NO AS FF_BUHIN_NO, " _
        '    & " AL.F_BUHIN_NO AS BF_BUHIN_NO, " _
        ...
        '    & "             And AL.OP_CODE = BASE.BASE_OP " _
        '    & "             AND AL.F_BUHIN_NO <>  '         0     ' " _
        '    & "             AND AL.SOBI_KAITEI_NO = " _
        '    & "                 RIGHT('000' + CAST(CAST(BASE.BASE_SOBI_KAITEI_NO as numeric(3))-1 AS VarChar),3) " _
        '    & "             AND LEFT(AL.BLOCK_NO,3) <>  '999' " _
        '    & "     LEFT OUTER JOIN " _
        '    & "         " & RHACLIBF_DB_NAME & ".dbo.RHAC0080 BLOCKM ON " _
        '    & "             BLOCKM.BLOCK_NO_KINO = AL.BLOCK_NO " _
        ...
        '    & "         AND SOBI_KAITEI_NO = AL.SOBI_KAITEI_NO ) " _
        '    & "         AND NOT EXISTS ( " _
        '    & "                 SELECT SHISAKU_BLOCK_NO FROM " & MBOM_DB_NAME & ".dbo.T_SHISAKU_SEKKEI_BLOCK_INSTL " _
        '    & "                         WHERE SHISAKU_BLOCK_NO = AL.BLOCK_NO " _
        '    & "                             And SHISAKU_GOUSYA = BASE.SHISAKU_GOUSYA " _
        '    & "                             AND SHISAKU_EVENT_CODE = @Value ) " _
        '    & " GROUP BY " _
        '    & "     AL.BLOCK_NO, " _
        '    & "     AL.F_BUHIN_NO, " _
        '    & "     KA.BU_CODE, " _
        ...
        ''' <remarks></remarks>
        Public Function FindShisakuBlockInstlByShisakuEventBase(ByVal shisakuEventCode As String, ByVal unitKbn As String, ByVal HyojiJunNo As String, Optional ByVal flg As Boolean = False) As List(Of SekkeiBlockInstlResultVo) Implements SekkeiBlockDao.FindShisakuBlockInstlByShisakuEventBase
            Dim sql As New StringBuilder()
            '2012/02/16 ユニット区分を抽出条件に追加
            ' > ベース車の、ベース試作イベントコード・ベース号車の選択値用のメソッド
            ' > EventEditBaseCarDaoImpl#FindBlockInstlForLabelValuesBy 
            ' 上記は似たようなSQLをもつメソッド
            ' 当SQL修正時は、上記メソッドも恐らく修正必要
            Dim tmpUnitKbn As String = ""
            With sql
                .Remove(0, .Length)
                .AppendLine("SELECT I.SHISAKU_EVENT_CODE, ")
                If flg Then
                ...
                .AppendLine(" I.SHISAKU_BLOCK_NO_KAITEI_NO, ")
                .AppendLine(" I.SHISAKU_GOUSYA, ")
                .AppendLine(" I.INSTL_HINBAN_HYOUJI_JUN, ")
                .AppendLine(" I.INSTL_HINBAN, ")
                .AppendLine(" I.INSTL_HINBAN_KBN, ")
                .AppendLine(" I.INSTL_DATA_KBN, ")
                .AppendLine(" I.BASE_INSTL_FLG, ")
                .AppendLine(" I.BF_BUHIN_NO, ")
                .AppendLine(" I.INSU_SURYO, ")
                .AppendLine(" B.SHISAKU_BLOCK_NAME, E.SHISAKU_GOUSYA BASE_SHISAKU_GOUSYA ")
                .AppendLine(" ,SV.SHISAKU_KAIHATSU_FUGO")
                .AppendLine("FROM " & MBOM_DB_NAME & ".dbo.T_SHISAKU_SEKKEI_BLOCK_INSTL I ")
                .AppendLine(" WITH (NOLOCK, NOWAIT)  ")

                .AppendLine("    INNER JOIN " & MBOM_DB_NAME & ".dbo.T_SHISAKU_SEKKEI_BLOCK B ")
                .AppendLine("        ON I.SHISAKU_EVENT_CODE = B.SHISAKU_EVENT_CODE ")
                .AppendLine("        AND I.SHISAKU_BUKA_CODE = B.SHISAKU_BUKA_CODE ")
                .AppendLine("        AND I.SHISAKU_BLOCK_NO = B.SHISAKU_BLOCK_NO ")
                .AppendLine("        AND I.SHISAKU_BLOCK_NO_KAITEI_NO = B.SHISAKU_BLOCK_NO_KAITEI_NO ")

                .AppendLine("    INNER JOIN " & MBOM_DB_NAME & ".dbo.T_SHISAKU_EVENT_BASE E ")
                .AppendLine("        ON I.SHISAKU_EVENT_CODE = E.SHISAKU_BASE_EVENT_CODE ")
                .AppendLine("        AND I.SHISAKU_GOUSYA = E.SHISAKU_BASE_GOUSYA ")
                .AppendLine("    INNER JOIN " & MBOM_DB_NAME & ".dbo.T_SHISAKU_EVENT SV ")
                .AppendLine("        ON I.SHISAKU_EVENT_CODE = SV.SHISAKU_EVENT_CODE ")
                If flg Then
                Else
                    .AppendLine("    INNER JOIN " & MBOM_DB_NAME & ".dbo.T_SHISAKU_BLOCK_SEKKEIKA_TMP T ")
                    .AppendLine("        ON E.SHISAKU_BASE_EVENT_CODE = T.SHISAKU_EVENT_CODE ")
                    .AppendLine("        AND I.SHISAKU_BLOCK_NO = T.SHISAKU_BLOCK_NO ")
                End If
                .AppendLine("WHERE E.SHISAKU_EVENT_CODE = @ShisakuEventCode ")
                .AppendLine(" AND E.HYOJIJUN_NO = @HyojiJunNo ")
                'End If
                If unitKbn <> "S" Then
                    .AppendLine("        AND B.UNIT_KBN <> @UnitKbn ")
                    If unitKbn = "M" Then
                        tmpUnitKbn = "T"
                    Else
                        tmpUnitKbn = "M"
                    End If
                End If
                .AppendLine("    AND B.SHISAKU_BLOCK_NO_KAITEI_NO = ")
                .AppendLine("        (SELECT MAX(SHISAKU_BLOCK_NO_KAITEI_NO) FROM " & MBOM_DB_NAME & ".dbo.T_SHISAKU_SEKKEI_BLOCK WITH (NOLOCK, NOWAIT) ")
                .AppendLine("        WHERE SHISAKU_EVENT_CODE = I.SHISAKU_EVENT_CODE ")
                .AppendLine("            AND SHISAKU_BUKA_CODE = I.SHISAKU_BUKA_CODE ")
                .AppendLine("            AND SHISAKU_BLOCK_NO = I.SHISAKU_BLOCK_NO) ")
                '2012/02/22 ここにブロック不要で無いという条件を追加
                .AppendLine("    AND B.BLOCK_FUYOU = 0 ")
                .AppendLine("    AND EXISTS (SELECT * FROM " & MBOM_DB_NAME & ".dbo.T_SHISAKU_BUHIN_EDIT_INSTL WITH (NOLOCK, NOWAIT) ")
                .AppendLine("        WHERE SHISAKU_EVENT_CODE = I.SHISAKU_EVENT_CODE ")
                .AppendLine("            AND SHISAKU_BUKA_CODE = I.SHISAKU_BUKA_CODE ")
                .AppendLine("            AND SHISAKU_BLOCK_NO = I.SHISAKU_BLOCK_NO ")
                .AppendLine("            AND SHISAKU_BLOCK_NO_KAITEI_NO = I.SHISAKU_BLOCK_NO_KAITEI_NO ")
                .AppendLine("            AND INSTL_HINBAN_HYOUJI_JUN = I.INSTL_HINBAN_HYOUJI_JUN ")
                .AppendLine("        )")
            End With
            Dim db As New EBomDbClient
            Dim param As New SekkeiBlockAlConditionVo(shisakuEventCode, HyojiJunNo, tmpUnitKbn)
            Return db.QueryForList(Of SekkeiBlockInstlResultVo)(sql.ToString(), param)
        End Function
        ...
        Public Function FindSekkeika(ByVal eventCode As String, ByVal shisakuBlockNo As String) As List(Of TShisakuBlockSekkeikaTmpVo) Implements SekkeiBlockDao.FindSekkeika
            Dim sql As New StringBuilder()
            With sql
                .Remove(0, .Length)
                .AppendLine("SELECT * ")
            ...
            Return db.QueryForList(Of TShisakuBlockSekkeikaTmpVo)(sql.ToString(), param)
        ...
        ''' <returns>該当した「試作設計ブロックINSTL情報」</returns>
        ''' <remarks></remarks>
        Public Function FindShisakuBlockInstlByShisakuEventBaseByBlock(ByVal shisakuEventCode As String, ByVal unitKbn As String, ByVal blockNo As String) As List(Of SekkeiBlockInstlResultVo) Implements SekkeiBlockDao.FindShisakuBlockInstlByShisakuEventBaseByBlock
            Dim sql As New StringBuilder()
            '2012/02/16 ユニット区分を抽出条件に追加
            ' > ベース車の、ベース試作イベントコード・ベース号車の選択値用のメソッド
            ' > EventEditBaseCarDaoImpl#FindBlockInstlForLabelValuesBy 
            ...
            With sql
                ...
                .AppendLine("                 SBI.CREATED_USER_ID,  ")
                .AppendLine("                 SBI.CREATED_DATE,  ")
                .AppendLine("                 SBI.CREATED_TIME,  ")
                .AppendLine("                 SBI.UPDATED_USER_ID,  ")
                .AppendLine("                 SBI.UPDATED_DATE,  ")
                .AppendLine("                 SBI.UPDATED_TIME,  ")
                .AppendLine(" SB.SHISAKU_BLOCK_NAME, B.SHISAKU_GOUSYA BASE_SHISAKU_GOUSYA ")
                ...
                If StringUtil.IsNotEmpty(blockNo) Then
                    .AppendLine(" AND SBI.SHISAKU_BLOCK_NO = @BlockNo ")
                End If
                .AppendLine(" AND SBI.SHISAKU_BLOCK_NO_KAITEI_NO = ")
                .AppendLine(" (SELECT MAX(SHISAKU_BLOCK_NO_KAITEI_NO) FROM " & MBOM_DB_NAME & ".dbo.T_SHISAKU_SEKKEI_BLOCK_INSTL WITH (NOLOCK, NOWAIT) ")
                ...
                .AppendLine(" ON SB.SHISAKU_EVENT_CODE = B.SHISAKU_BASE_EVENT_CODE ")
                ...
                .AppendLine(" INNER JOIN " & MBOM_DB_NAME & ".dbo.T_SHISAKU_BLOCK_SEKKEIKA_TMP TMP ")
                .AppendLine(" ON TMP.SHISAKU_EVENT_CODE = B.SHISAKU_EVENT_CODE ")
                .AppendLine(" AND TMP.SHISAKU_BLOCK_NO = SBI.SHISAKU_BLOCK_NO ")

                If unitKbn <> "S" Then
                    If unitKbn = "M" Then
                        .AppendLine("        AND SB.UNIT_KBN <> 'T' ")
                    Else
                        .AppendLine("        AND SB.UNIT_KBN <> 'M' ")
                    End If
                End If
                ...
                .AppendLine(" WHERE B.SHISAKU_EVENT_CODE = @ShisakuEventCode ")
                ''2015/09/01 追加 E.Ubukata Ver2.11.0
                '' 上記条件だけでは移管車改修のイベントで別の列に定義されているものが同一に扱われてしまうため追加
                '.AppendLine(" ORDER BY SBI.SHISAKU_BLOCK_NO,SBI.INSTL_HINBAN,SBI.INSTL_HINBAN_KBN,SBI.INSTL_DATA_KBN ")
                .AppendLine(" ORDER BY SBI.SHISAKU_BLOCK_NO,SBI.INSTL_HINBAN,SBI.INSTL_HINBAN_KBN,SBI.INSTL_DATA_KBN,SBI.INSTL_HINBAN_HYOUJI_JUN ")
            End With
            Dim db As New EBomDbClient
            Dim param As New SekkeiBlockAlConditionVo(shisakuEventCode, blockNo)
            Return db.QueryForList(Of SekkeiBlockInstlResultVo)(sql.ToString(), param)
        ...
        ''' <remarks></remarks>
        Function FindByShisakuEventBase(ByVal shisakuEventCode As String) As List(Of TShisakuEventBaseVo) Implements SekkeiBlockDao.FindByShisakuEventBase
            Dim sql As New StringBuilder()
            With sql
                .Remove(0, .Length)
                .AppendLine("SELECT ")
        ...
        Function FindTantoBushoByBlock(ByVal KaihatsuFugo As String, _
                                      ByVal BlockNoKino As String) As Rhac0080Vo Implements SekkeiBlockDao.FindTantoBushoByBlock
            Dim sql As New StringBuilder()
            With sql
                .Remove(0, .Length)
                .AppendLine(" SELECT * ")
                .AppendLine(" FROM " & RHACLIBF_DB_NAME & ".dbo.Rhac0080 WITH (NOLOCK, NOWAIT) ")
                .AppendLine(" WHERE KAIHATSU_FUGO = '" & KaihatsuFugo & "' AND ")
                .AppendLine("       BLOCK_NO_KINO = '" & BlockNoKino & "' AND ")
                .AppendLine("       KAITEI_NO_KINO = (SELECT MAX(KAITEI_NO_KINO) FROM " & RHACLIBF_DB_NAME & ".dbo.Rhac0080 WITH (NOLOCK, NOWAIT) WHERE KAIHATSU_FUGO = '" & KaihatsuFugo & "' AND BLOCK_NO_KINO = '" & BlockNoKino & "' ) ")
            End With
            Dim db As New EBomDbClient
            Return db.QueryForObject(Of Rhac0080Vo)(sql.ToString())
        End Function
...
#Region "親品番が自給品のデータを削除する"
        ...
        Public Function FindSekkeika4Update(ByVal shisakuEventCode As String, Optional ByVal baseFlg As Boolean = False) As List(Of ShisakuSekkeiBlock4EbomVo) Implements SekkeiBlockDao.FindSekkeika4Update
            Dim sb As New StringBuilder
            With sb
                .Remove(0, .Length)
                .AppendLine(" SELECT *, TMP.SHISAKU_BUKA_CODE AS SHISAKU_BUKA_CODE_NEW ")
                .AppendLine(" FROM " & MBOM_DB_NAME & ".dbo.T_SHISAKU_SEKKEI_BLOCK SB ")
                .AppendLine(" INNER JOIN " & MBOM_DB_NAME & ".dbo.T_SHISAKU_BLOCK_SEKKEIKA_TMP TMP ")
                .AppendLine(" ON SB.SHISAKU_EVENT_CODE = TMP.SHISAKU_EVENT_CODE ")
                .AppendLine(" AND SB.SHISAKU_BLOCK_NO = TMP.SHISAKU_BLOCK_NO ")
                .AppendLine(" AND NOT SB.SHISAKU_BUKA_CODE = TMP.SHISAKU_BUKA_CODE ")
                .AppendLine(" WHERE ")
                .AppendLine(" SB.SHISAKU_EVENT_CODE = @ShisakuEventCode ")
                .AppendLine(" AND SB.SHISAKU_BLOCK_NO_KAITEI_NO = @ShisakuBlockNoKaiteiNo ")
            End With

            Dim param As New ShisakuSekkeiBlock4EbomVo
            param.ShisakuEventCode = shisakuEventCode
            'If baseFlg Then
            ...
            Dim db As New EBomDbClient
            Return db.QueryForList(Of ShisakuSekkeiBlock4EbomVo)(sb.ToString(), param)

        End Function
        Public Function FindShisakuSekkeiBlockByShisakuBlockNoHyoujiJun(ByVal shisakuEventCode As String, ByVal shisakuBukaCode As String, ByVal shisakuBlockNo As String, ByVal shisakuBlockNoKaiteiNo As String) As List(Of TShisakuSekkeiBlockVo) Implements SekkeiBlockDao.FindShisakuSekkeiBlockByShisakuBlockNoHyoujiJun
            Dim sb As New StringBuilder
            With sb
                .Remove(0, .Length)
                .AppendLine(" SELECT * ")
            ...
            Dim db As New EBomDbClient
            Return db.QueryForList(Of TShisakuSekkeiBlockVo)(sb.ToString(), param)
        End Function

        Public Sub UpdateShisakuSekkeiBlockByShisakuBlockNoHyoujiJun(ByVal shisakuEventCode As String, ByVal shisakuBlockNoKaiteiNo As String, ByVal shisakuBukaCode As String, ByVal shisakuBlockNoHyoujiJun As Integer) Implements SekkeiBlockDao.UpdateShisakuSekkeiBlockByShisakuBlockNoHyoujiJun
            Dim db As New EBomDbClient

            Dim sb As New StringBuilder
            With sb
                .Remove(0, .Length)
                .AppendLine(" UPDATE " & MBOM_DB_NAME & ".dbo.T_SHISAKU_SEKKEI_BLOCK ")
                .AppendLine(" SET SHISAKU_BLOCK_NO_HYOUJI_JUN = SHISAKU_BLOCK_NO_HYOUJI_JUN + 1 ")
                .AppendLine(" WHERE SHISAKU_EVENT_CODE = @ShisakuEventCode ")
                .AppendLine(" AND SHISAKU_BUKA_CODE = @ShisakuBukaCode  ")
                .AppendLine(" AND SHISAKU_BLOCK_NO_KAITEI_NO = @ShisakuBlockNoKaiteiNo  ")
                .AppendLine(" AND SHISAKU_BLOCK_NO_HYOUJI_JUN >= @ShisakuBlockNoHyoujiJun  ")
            End With
            ...
            param.ShisakuBukaCode = shisakuBukaCode
            param.ShisakuBlockNoKaiteiNo = shisakuBlockNoKaiteiNo
            param.ShisakuBlockNoHyoujiJun = shisakuBlockNoHyoujiJun

            db.Update(sb.ToString, param)

        End Sub
        Public Sub UpdateShisakuSekkeiBlockByShisakuBlockNoHyoujiJun2(ByVal shisakuEventCode As String, ByVal shisakuBlockNoKaiteiNo As String, ByVal shisakuBukaCode As String, ByVal shisakuBlockNoHyoujiJun As Integer) Implements SekkeiBlockDao.UpdateShisakuSekkeiBlockByShisakuBlockNoHyoujiJun2
            Dim db As New EBomDbClient
        ...
        End Sub
        Public Sub UpdateShisakuSekkeiBlockByShisakuBukaCode(ByVal shisakuEventCode As String, ByVal shisakuBukaCode As String, ByVal shisakuBlockNo As String, ByVal shisakuBlockNoKaiteiNo As String, ByVal shisakuBukaCodeNew As String, ByVal shisakuBlockNoHyoujiJunNew As Integer) Implements SekkeiBlockDao.UpdateShisakuSekkeiBlockByShisakuBukaCode
            Dim db As New EBomDbClient
            ...
            With sb
                .Remove(0, .Length)
                .AppendLine(" UPDATE " & MBOM_DB_NAME & ".dbo.T_SHISAKU_SEKKEI_BLOCK ")
                .AppendLine(" SET SHISAKU_BUKA_CODE = @ShisakuBukaCodeNew , ")
                .AppendLine(" SHISAKU_BLOCK_NO_HYOUJI_JUN = @ShisakuBlockNoHyoujiJunNew ")
                .AppendLine(" WHERE SHISAKU_EVENT_CODE = @ShisakuEventCode ")
                .AppendLine(" AND SHISAKU_BUKA_CODE = @ShisakuBukaCode  ")
                .AppendLine(" AND SHISAKU_BLOCK_NO = @ShisakuBlockNo  ")
                .AppendLine(" AND SHISAKU_BLOCK_NO_KAITEI_NO = @ShisakuBlockNoKaiteiNo  ")
            ...
            Dim param As New Block4UpdateBlockNoHyoujiJunVo
            param.ShisakuEventCode = shisakuEventCode
            param.ShisakuBukaCode = shisakuBukaCode
            param.ShisakuBlockNo = shisakuBlockNo
            param.ShisakuBlockNoKaiteiNo = shisakuBlockNoKaiteiNo
            param.ShisakuBukaCodeNew = shisakuBukaCodeNew
            param.shisakuBlockNoHyoujiJunNew = shisakuBlockNoHyoujiJunNew

            db.Update(sb.ToString, param)
        End Sub
        'Public Function FindShisakuSekkeiBlockInstlByBukaBlock(ByVal shisakuEventCode As String, ByVal shisakuBukaCode As String, ByVal shisakuBlockNo As String, ByVal shisakuBlockNoKaiteiNo As String) As List(Of TShisakuSekkeiBlockInstlVo) Implements SekkeiBlockDao.FindShisakuSekkeiBlockInstlByBukaBlock
        '    Dim sb As New StringBuilder
        '    With sb
        '        .Remove(0, .Length)
        '        .AppendLine(" SELECT DISTINCT  SBI.SHISAKU_EVENT_CODE, SBI.SHISAKU_BUKA_CODE, SBI.SHISAKU_BLOCK_NO, SBI.SHISAKU_BLOCK_NO_KAITEI_NO, SBI.SHISAKU_GOUSYA ")
        ...
        Public Function FindShisakuSekkeiBlockInstlByInstlHinbanHyoujiJun(ByVal shisakuEventCode As String, ByVal shisakuBukaCode As String, ByVal shisakuBlockNo As String, ByVal shisakuBlockNoKaiteiNo As String) As List(Of TShisakuSekkeiBlockInstlVo) Implements SekkeiBlockDao.FindShisakuSekkeiBlockInstlByInstlHinbanHyoujiJun
            Dim sb As New StringBuilder
            With sb
                .Remove(0, .Length)
                .AppendLine(" SELECT * ")
                ...
                .AppendLine(" AND SBI.SHISAKU_BLOCK_NO_KAITEI_NO = @ShisakuBlockNoKaiteiNo ")
                .AppendLine(" AND SBI.SHISAKU_BLOCK_NO = @ShisakuBlockNo ")
                .AppendLine(" AND SBI.INSTL_HINBAN_HYOUJI_JUN = ")
                .AppendLine(" (")
                .AppendLine(" SELECT MAX(INSTL_HINBAN_HYOUJI_JUN) FROM " & MBOM_DB_NAME & ".dbo.T_SHISAKU_SEKKEI_BLOCK_INSTL ")
                .AppendLine(" WHERE ")
        ...
        Public Sub UpdateShisakuSekkeiBlockInstlByInstlHinbanHyoujiJun(ByVal shisakuEventCode As String, ByVal shisakuBukaCode As String, ByVal shisakuBlockNo As String, ByVal shisakuBlockNoKaiteiNo As String, ByVal shisakuBukaCodeNew As String, ByVal instlHinbanHyoujiJunNew As Integer) Implements SekkeiBlockDao.UpdateShisakuSekkeiBlockInstlByInstlHinbanHyoujiJun
            Dim db As New EBomDbClient
            ...
            With sb
                ...
                .AppendLine(" SET SHISAKU_BUKA_CODE = @ShisakuBukaCodeNew , ")
                .AppendLine(" INSTL_HINBAN_HYOUJI_JUN = INSTL_HINBAN_HYOUJI_JUN + @InstlHinbanHyoujiJunNew ")
                .AppendLine(" WHERE SHISAKU_EVENT_CODE = @ShisakuEventCode ")
                .AppendLine(" AND SHISAKU_BUKA_CODE = @ShisakuBukaCode  ")
                .AppendLine(" AND SHISAKU_BLOCK_NO = @ShisakuBlockNo  ")
                .AppendLine(" AND SHISAKU_BLOCK_NO_KAITEI_NO = @ShisakuBlockNoKaiteiNo  ")
            ...
            Dim param As New BlockInstl4UpdateInstlHinbanHyoujiJunVo
            param.ShisakuEventCode = shisakuEventCode
            param.ShisakuBukaCode = shisakuBukaCode
            param.ShisakuBlockNo = shisakuBlockNo
            param.ShisakuBlockNoKaiteiNo = shisakuBlockNoKaiteiNo
            param.ShisakuBukaCodeNew = shisakuBukaCodeNew
            param.InstlHinbanHyoujiJunNew = instlHinbanHyoujiJunNew

            db.Update(sb.ToString, param)
        End Sub
        Public Function FindShisakuBuhinEditByBuhinNoHyoujiJun(ByVal shisakuEventCode As String, ByVal shisakuBukaCode As String, ByVal shisakuBlockNo As String, ByVal shisakuBlockNoKaiteiNo As String, Optional ByVal baseFlg As Boolean = False) As List(Of TShisakuBuhinEditVo) Implements SekkeiBlockDao.FindShisakuBuhinEditByBuhinNoHyoujiJun
            Dim sb As New StringBuilder
            With sb
                .Remove(0, .Length)
                .AppendLine(" SELECT * ")
                ...
                .AppendLine(" AND BE.BUHIN_NO_HYOUJI_JUN = ")
                .AppendLine(" (")
                If baseFlg Then
                    .AppendLine(" SELECT MAX(BUHIN_NO_HYOUJI_JUN) FROM " & MBOM_DB_NAME & ".dbo.T_SHISAKU_BUHIN_EDIT_BASE ")
                Else
                    .AppendLine(" SELECT MAX(BUHIN_NO_HYOUJI_JUN) FROM " & MBOM_DB_NAME & ".dbo.T_SHISAKU_BUHIN_EDIT ")
                End If
                .AppendLine(" WHERE ")
                .AppendLine(" SHISAKU_EVENT_CODE = @ShisakuEventCode ")
                .AppendLine(" AND SHISAKU_BUKA_CODE = @ShisakuBukaCode ")
        ...
        End Function
        Public Sub UpdateShisakuBuhinEditByBuhinNoHyoujiJun(ByVal shisakuEventCode As String, ByVal shisakuBukaCode As String, ByVal shisakuBlockNo As String, ByVal shisakuBlockNoKaiteiNo As String, ByVal shisakuBukaCodeNew As String, ByVal BuhinNoHyoujiJunNew As Integer, Optional ByVal baseFlg As Boolean = False) Implements SekkeiBlockDao.UpdateShisakuBuhinEditByBuhinNoHyoujiJun
            Dim db As New EBomDbClient
            ...
            With sb
                ...
                End If
                .AppendLine(" SET SHISAKU_BUKA_CODE = @ShisakuBukaCodeNew , ")
                .AppendLine(" BUHIN_NO_HYOUJI_JUN = BUHIN_NO_HYOUJI_JUN + @BuhinNoHyoujiJunNew ")
                .AppendLine(" WHERE SHISAKU_EVENT_CODE = @ShisakuEventCode ")
                .AppendLine(" AND SHISAKU_BUKA_CODE = @ShisakuBukaCode  ")
                .AppendLine(" AND SHISAKU_BLOCK_NO = @ShisakuBlockNo  ")
                .AppendLine(" AND SHISAKU_BLOCK_NO_KAITEI_NO = @ShisakuBlockNoKaiteiNo  ")
            ...
            Dim param As New Buhin4UpdateBukaCodeBuhinNoHyoujiJun
            param.ShisakuEventCode = shisakuEventCode
            param.ShisakuBukaCode = shisakuBukaCode
            param.ShisakuBlockNo = shisakuBlockNo
            If baseFlg Then
            ...
            End If
            param.ShisakuBukaCodeNew = shisakuBukaCodeNew
            param.BuhinNoHyoujiJunNew = BuhinNoHyoujiJunNew

            db.Update(sb.ToString, param)
        End Sub
        Public Sub UpdateShisakuBuhinEditInstlByBuhinJunInstlJun(ByVal shisakuEventCode As String, ByVal shisakuBukaCode As String, ByVal shisakuBlockNo As String, ByVal shisakuBlockNoKaiteiNo As String, ByVal shisakuBukaCodeNew As String, ByVal BuhinNoHyoujiJunNew As Integer, ByVal instlHinbanHyoujiJunNew As Integer, Optional ByVal baseFlg As Boolean = False) Implements SekkeiBlockDao.UpdateShisakuBuhinEditInstlByBuhinJunInstlJun
            Dim db As New EBomDbClient
            ...
            Dim param As New BuhinInstl4UpdateBukaBuhinJunInstlJun
            param.ShisakuEventCode = shisakuEventCode
            param.ShisakuBukaCode = shisakuBukaCode
            param.ShisakuBlockNo = shisakuBlockNo
            If baseFlg Then
