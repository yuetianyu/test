...
#End Region
        ...
        Public Sub New(ByVal shisakuEventCode As String, _
            ...
            If IsAddMode() Then
                ...
                '発行№の指定がある場合、製作一覧ＤＢから取込
                If StringUtil.IsNotEmpty(seisakuHakouNo) Then
                    '' 製作一覧発行№  
                    _SeisakuichiranHakouNo = seisakuHakouNo
                    '' 製作一覧発行№改訂  
                    _SeisakuichiranHakouNoKai = seisakuHakouNoKaiteiNo
                    '開発符号
                    _ShisakuKaihatsuFugo = tSeisakuHakouHdVo.KaihatsuFugo
                    'イベントフェーズ名のラベルを設定
                    ShisakuEventPhaseNameLabelValues = GetLabelValues_EventPhaseName()

                    'イベント
                    _ShisakuEventPhaseName = tSeisakuHakouHdVo.SeisakuEvent
                    'イベント名
                    _ShisakuEventName = tSeisakuHakouHdVo.SeisakuEventName

                    '以下のセットは不要かな？
                    ''台数・完成車
                    '_SeisakudaisuKanseisya = tSeisakuHakouHdVo.SeisakudaisuKanseisya
                    ''台数・ＷＢ車
                    '_SeisakudaisuWb = tSeisakuHakouHdVo.SeisakudaisuWb
                    ''台数・中止
                    '_SeisakudaisuChushi = 0
            ...
            Else
                ...
                'If StringUtil.Equals(tSeisakuHakouHdVo, Nothing) = False Then
                '    '開発符号
                '    _ShisakuKaihatsuFugo = tSeisakuHakouHdVo.KaihatsuFugo
                '    'イベント
                '    _ShisakuEventPhaseName = tSeisakuHakouHdVo.SeisakuEvent
        ...
        Private Sub RegisterMain(ByVal isRegister As Boolean)
            Dim value As TShisakuEventVo
            If IsAddMode() Then
                value = New TShisakuEventVo
            Else
                value = Me.aEventDao.FindByPk(_shisakuEventCode)
            End If

            Dim param As New MShisakuKaihatufugoVo
            param.ShisakuKaihatsuFugo = ShisakuKaihatsuFugo
            param.ShisakuEventPhaseName = ShisakuEventPhaseName
            Dim aMShisakuKaihatufugoVo As MShisakuKaihatufugoVo = aKaihatufugoDao.FindBy(param)(0)
            If IsAddMode() Then
                Dim numbering As New NumberingShisakuEventCode(aEventDao, ShisakuKaihatsuFugo, aMShisakuKaihatufugoVo.ShisakuEventPhase, UnitKbn)
                value.ShisakuEventCode = numbering.NextShisakuEventCode
            Else
                value.ShisakuEventCode = _shisakuEventCode
            End If
            value.ShisakuShakeiCode = aMShisakuKaihatufugoVo.ShisakuShakeiCode
            value.ShisakuKaihatsuFugo = ShisakuKaihatsuFugo
            value.HyojijunNo = aMShisakuKaihatufugoVo.HyojijunNo
            value.ShisakuEventPhase = aMShisakuKaihatufugoVo.ShisakuEventPhase
            value.ShisakuEventPhaseName = ShisakuEventPhaseName
            value.UnitKbn = UnitKbn
            value.ShisakuEventName = ShisakuEventName
            value.SeisakujikiFrom = DateUtil.ConvDateToIneteger(SeisakujikiFrom)
            value.SeisakujikiTo = DateUtil.ConvDateToIneteger(SeisakujikiTo)
            value.SeisakudaisuKanseisya = StringUtil.ToInteger(SeisakudaisuKanseisya)
            value.SeisakudaisuWb = StringUtil.ToInteger(SeisakudaisuWb)
            value.SeisakudaisuChushi = StringUtil.ToInteger(SeisakudaisuChushi)
            value.SeisakuichiranHakouNo = SeisakuichiranHakouNo
            value.SeisakuichiranHakouNoKai = StringUtil.ToInteger(SeisakuichiranHakouNoKai)
            value.HachuUmu = HachuUmu
            value.JikyuUmu = JikyuUmu       '2012/01/10
            value.EventTourokuTantoKa = login.KaCode
            value.EventTourokuTanto = login.UserId
            value.DataKbn = IIf(isRegister, _
                                TShisakuEventVoHelper.DataKbn.REGISTER, _
                                TShisakuEventVoHelper.DataKbn.SAVE)

            If IsAddMode() Then
                value.CreatedUserId = login.UserId
                value.CreatedDate = aDate.CurrentDateDbFormat
                value.CreatedTime = aDate.CurrentTimeDbFormat
                value.RegisterDate = aDate.CurrentDateDbFormat
                value.RegisterTime = aDate.CurrentTimeDbFormat
                value.TenkaiStatus = "0"
            End If
            value.UpdatedUserId = login.UserId
            value.UpdatedDate = aDate.CurrentDateDbFormat
            value.UpdatedTime = aDate.CurrentTimeDbFormat

            ''↓↓2014/09/17 Ⅰ.3.設計編集 ベース改修専用化_a) 酒井 ADD BEGIN
            value.KounyuShijiFlg = "0"
            ''↑↑2014/09/17 Ⅰ.3.設計編集 ベース改修専用化_a) 酒井 ADD END
            ...
            Else
                '保存フラグを更新
                value.SaveFlg = "1" '保存した。
            End If

            If IsAddMode() Then
                aEventDao.InsertBy(value)
            Else
                aEventDao.UpdateByPk(value)
            End If

            _shisakuEventCode = value.ShisakuEventCode

            If Not isRegister Then
                Dim vo As MShisakuStatusVo = aStatusDao.FindByPk(value.DataKbn)
                If vo IsNot Nothing Then
                    _dataKbnName = vo.ShisakuStatusName
                    SetChanged()
                End If
            End If
        End Sub
