...
Namespace YosanSetteiBuhinSakusei.Logic
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Class YosanSetteiBuhinSakusei : Inherits Observable
        ...
        Private ReadOnly yosanSetteiBuhinSakusei As YosanSetteiBuhinSakuseiDao
        Private aDate As ShisakuDate
        Private aList As TYosanSetteiListcodeDao
        Private ListCode As String
        Private KaihatsuFugo As String
        Private hikakuImpl As YosanSetteiBuhinHikakuDao = New YosanSetteiBuhinHikakuDaoImpl
        ...
        Public Sub New(ByVal aLoginInfo As LoginInfo, ByVal shisakuEventCode As String)
            ...
            yosanSetteiBuhinSakusei = New YosanSetteiBuhinSakuseiDaoImpl
            'グループNo'
            ...
            Kanseivo = yosanSetteiBuhinSakusei.FindByGroupNo(shisakuEventCode)

            Dim Eventvo = yosanSetteiBuhinSakusei.FindByEventName(shisakuEventCode)

            Dim Seisakuvo As AsSKMSVo = yosanSetteiBuhinSakusei.FindByAsSNKMOne()
        ...
        Private Sub RegisterMain()
            ...
            Try
                ...
                Dim MergeList As New List(Of YosanSetteiBuhinEditTmpVo)
                Dim DummyVo As New TYosanSetteiGousyaVo
                ...
                For Each Vo As TShisakuSekkeiBlockVo In BlockListVo
                    ...
                    If Not MergeList.Count = 0 Then
                        ...
                        Dim NewMergeList As New List(Of YosanSetteiBuhinEditTmpVo)
                        ...
                        DummyVo.YosanBukaCode = MergeList(0).ShisakuBukaCode
                        DummyVo.YosanBlockNo = MergeList(0).ShisakuBlockNo
                    End If
                Next
                ...
                For Each baseVo As TShisakuEventBaseVo In baseList
                    Dim DummyGousyaVo As New TYosanSetteiGousyaVo
                    DummyGousyaVo.ShisakuEventCode = ShisakuEventCode
                    DummyGousyaVo.YosanListCode = ListCode
                    DummyGousyaVo.YosanBukaCode = ""
                    DummyGousyaVo.YosanBlockNo = ""
                    DummyGousyaVo.YosanGousya = baseVo.ShisakuGousya
                    DummyGousyaVo.YosanGousyaHyoujiJun = baseVo.HyojijunNo
                ...
                Dim DummyGousyaVo2 As New TYosanSetteiGousyaVo
                ...
                Dim kaiteiDao As YosanSetteiBuhinSakuseiDao = New YosanSetteiBuhinSakuseiDaoImpl
                Dim maxGousyaHyoujiJun As Integer = kaiteiDao.FindByMaxGousyaHyoujijunNotDummy(ShisakuEventCode, ListCode)

                'DummyGousyaVo2がNothingの時がある。
                If StringUtil.IsNotEmpty(DummyGousyaVo2) Then

                    DummyGousyaVo2.YosanGousyaHyoujiJun = maxGousyaHyoujiJun

                    impl.InserByDummyTehaiGousya(DummyGousyaVo2)

                End If
                '行IDがずれるから振り直す'
                Dim kihonList As New List(Of TYosanSetteiBuhinVo)
...
#Region "各テーブルの操作"
        ...
        Private Sub getListCode()
            ...
            Dim ListCodeCheck = yosanSetteiBuhinSakusei.FindByListCode(_ShisakuEventCode, _KoujiShireiNo)
            If StringUtil.IsEmpty(ListCodeCheck.YosanListCode) Then
                ListCode = _ShisakuEventCode + "-" + "Y-" + _KoujiShireiNo + "01"
            Else
                Dim newListCode As Integer = Integer.Parse(ListCodeCheck.YosanListCode) + 1
                ListCode = _ShisakuEventCode + "-" + "Y-" + _KoujiShireiNo + Right("00" + newListCode.ToString(), 2)
            End If
        End Sub

        Private Sub InsertListCode()
            'とりあえずリストコードだけ作る'
            aList = New TYosanSetteiListcodeDaoImpl
            aDate = New ShisakuDate
            Dim value As New TYosanSetteiListcodeVo

            value.ShisakuEventCode = _ShisakuEventCode

            '試作イベントコードごとに連番'
            Dim hyoujijun = yosanSetteiBuhinSakusei.FindByHyoujijunNo(_ShisakuEventCode)
            If StringUtil.IsEmpty(hyoujijun.YosanListHyojijunNo) Then
                value.YosanListHyojijunNo = 0
            Else
                value.YosanListHyojijunNo = hyoujijun.YosanListHyojijunNo + 1
            End If

            value.YosanListCode = ListCode

            '試作グループNo'
            value.YosanGroupNo = _GroupNo

            '試作工事区分'
            value.YosanKoujiKbn = _KoujiKbn

            '試作製品区分'
            value.YosanSeihinKbn = _SeihinKbn

            '試作工事指令No'
            value.YosanKoujiShireiNo = _KoujiShireiNo

            '試作工事No'
            value.YosanKoujiNo = _KoujiNo

            '試作イベント名称'
            value.YosanEventName = _EventName

            '試作自給品の消しこみ'
            If _JikyuSuru = True Then
                value.YosanJikyuhin = "1"
            Else
                value.YosanJikyuhin = "0"
            End If
            ...
            If _OrikomiSuru = True Then
                value.YosanHikakukekka = "1"
            Else
                value.YosanHikakukekka = "0"
            End If

            '試作集計コードからの展開'
            If _SyukeiSuru = True Then
                value.YosanSyuukeiCode = "1"
            Else
                value.YosanSyuukeiCode = "0"
            End If

            '試作台数'
            Dim Gosha As New YosanSetteiBuhinSakuseiDaoImpl
            ...
            For index As Integer = 0 To Goshavo.Count - 1
                ...
                End If
            Next
            value.YosanDaisu = kanseiDaisu.ToString() + "+" + WDaisu.ToString()
            '予算部品表作成日'
            value.YosanBuhinSakuseibi = Replace(aDate.CurrentDateDbFormat, "-", "")
            '予算部品表作成時間'
            value.YosanBuhinSakuseiJikan = Replace(aDate.CurrentTimeDbFormat, ":", "")
            'ステータス'
            value.Status = ""
            '最終最新化実行日'
            value.LastSaishinBi = 0
            '最終最新化実行時間'
            value.LastSaishinJikan = 0
            '最終割付実行日'
            value.LastWaritukeBi = 0
            '最終割付実行時間'
            value.LastWaritukeJikan = 0
            '最終ファンクション単価設定日'
            value.LastFunctionTankaSetteiBi = 0
            '最終ファンクション単価設定時間'
            value.LastFunctionTankaSetteiJikan = 0
            '最終単価自動設定日'
            value.LastTankaAutoSetteiBi = 0
            '最終単価自動設定時間'
            value.LastTankaAutoSetteiJikan = 0
            '最終EXCEL取込日'
            value.LastExcelImportBi = 0
            '最終EXCEL取込時間'
            value.LastExcelImportJikan = 0
            '最終部品費端数処理日'
            value.LastBuhinHiHasuBi = 0
            '最終部品費端数処理時間'
            value.LastBuhinHiHasuJikan = 0
            'メモ欄'
            value.YosanMemo = ""
            '作成ユーザーID'
        ...
        Private Sub InsertBuhinTmp(ByVal aAsKpsm10p As List(Of AsKPSM10PVo), _
            ...
            Dim tmpimpl As YosanSetteiBuhinSakuseiDao = New YosanSetteiBuhinSakuseiDaoImpl

            Dim aBuhinvo As New List(Of YosanSetteiBuhinEditTmpVo)
        ...
        Private Sub UpdateBuhinTmpHikakuTenkaiAri()
            Dim uTmpImpl As YosanSetteiBuhinSakuseiDao = New YosanSetteiBuhinSakuseiDaoImpl
            ...
            Dim tmpvo As New List(Of TYosanSetteiBuhinTmpVo)
            ...
            If _OrikomiSuru Then
                ...
                '比較結果織込み処理'
                Dim hikaku As New YosanSetteiBuhinHikaku(hikakuBuhinvo, ShisakuEventCode, ListCode, _SeihinKbn, _GroupNo, JikyuSuru)
...
#End Region
        ...
        ''' <remarks></remarks>
        Private Function Merge(ByVal MergeList As List(Of YosanSetteiBuhinEditTmpVo)) As List(Of YosanSetteiBuhinEditTmpVo)
                        ...
                        If MergeList(index).Level = MergeList(index2).Level Then
                            If MergeList(index).BuhinNo = MergeList(index2).BuhinNo Then
                                ...
                                If StringUtil.IsEmpty(MergeList(index2).ShukeiCode) Then
                                    MergeList(index2).ShukeiCode = ""
                                End If
                                If StringUtil.IsEmpty(MergeList(index).SiaShukeiCode) Then
                                    MergeList(index).SiaShukeiCode = ""
                                End If
                                If StringUtil.IsEmpty(MergeList(index2).SiaShukeiCode) Then
                                    MergeList(index2).SiaShukeiCode = ""
                                End If
                                '集計コード一致でマージ対象'
        ...
        Private Sub WriteConsoleMoto(ByVal index As Integer, ByVal MergeList As List(Of YosanSetteiBuhinEditTmpVo))
        ...
        Private Sub WriteConsoleSaki(ByVal index As Integer, ByVal index2 As Integer, ByVal MergeList As List(Of YosanSetteiBuhinEditTmpVo))
        ...
        Private Shared Function FindMergeListTemp(ByVal Vo As YosanSetteiBuhinEditTmpVo) As Boolean
        ...
        Private Sub MergeCore(ByVal kyoku1 As String, _
                              ...
                              ByRef MergeList As List(Of YosanSetteiBuhinEditTmpVo), _
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Private Function ExchangeBuhinEdit(ByVal BuhinEditTmpVoList As List(Of TYosanSetteiBuhinTmpVo)) As List(Of TShisakuBuhinEditVo)
            Dim BuhinEditVoList As New List(Of TShisakuBuhinEditVo)


            For Each buhinEditTmpvo As TYosanSetteiBuhinTmpVo In BuhinEditTmpVoList
                ...
                BuhinEditVo.BuhinNote = buhinEditTmpvo.BuhinNote
...
#Region "コンボボックスの処理"
        ...
        Public Function GetLabelValues_GroupNo() As List(Of LabelValueVo)
            ...
            results.AddRange(LabelValueExtracter(Of TShisakuEventKanseiVo).Extract(yosanSetteiBuhinSakusei.FindByGroupNoList(ShisakuEventCode), New GroupNoExtraction))

            results.Sort(New LabelValueComparer)

            results.Insert(0, lVo)
        ...
        Public Function GetLabelValues_SeihinKbn() As List(Of LabelValueVo)
            ...
            Dim results As List(Of LabelValueVo) = _
                LabelValueExtracter(Of AsSKMSVo).Extract(yosanSetteiBuhinSakusei.FindByAsSNKM, New SeihinKbnExtraction)
            results.Sort(New LabelValueComparer)
            Return results
        End Function
