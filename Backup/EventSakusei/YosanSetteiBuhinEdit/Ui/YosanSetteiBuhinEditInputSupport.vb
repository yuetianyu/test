...
Namespace YosanSetteiBuhinEdit.Ui
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Class YosanSetteiBuhinEditInputSupport
        ...
        Private Function GetActiveCellText() As String
            ...
            '初回1行目の部品を修正した場合にのみ以下の条件で抜ける。
            If StringUtil.IsEmpty(activeSheet) Then
                Return ""
            End If
        ...
        Private Sub TxtSupport_OnPreviewKeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.PreviewKeyDownEventArgs)
            ...
            If Not activeSheet Is Nothing Then
                ...
                ElseIf e.KeyValue = Keys.Left Then
                    'If (0 < activeSheet.ActiveColumnIndex()) Then
                    '    'axis = GetAxisLocator(activeSpread).GetAxis(activeSheet.ActiveRowIndex(), activeSheet.ActiveColumnIndex() - 1)
                    '    axis = GetAxisLocator(activeSpread).GetAxis(activeSheet.ActiveRowIndex(), activeSheet.ActiveColumnIndex())
                    'End If
        ...
        ''' <remarks></remarks>
        Public Overloads Sub SetActiveCell(ByVal row As Integer, ByVal column As Integer)
        ...
        ''' <param name="column">列index</param>
        ''' <remarks></remarks>
        Public Overloads Sub SetActiveCell(ByVal aSheet As FarPoint.Win.Spread.SheetView, ByVal row As Integer, ByVal column As Integer)

            activeSheet = aSheet

            aSheet.SetActiveCell(row, column)
            'activeSpread.ShowActiveCell(VerticalPosition.Nearest, HorizontalPosition.Nearest)
            ' セルカーソルは移動しているけど、なぜか Selection は以前のまま移動しないので、Selection も再設定する
            aSheet.AddSelection(row, column, 1, 1)
            PerformPostMoveCell(row, column)
        End Sub
