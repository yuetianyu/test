...
Namespace YosanBuhinEdit.Kosei.Logic.Matrix
    ...
    Public Class BuhinKoseiMatrix
        ...
        Private Class RecordBag
            ...
            ''' <remarks></remarks>
            Public Sub New(ByVal KoseiRecordVo As BuhinKoseiRecordVo)
        ...
        Private _columnBags As New IndexedList(Of InstlColumnBag)
        ...
        ''' <remarks></remarks>
        Public Sub New(ByVal editVos As List(Of TYosanBuhinEditVo), ByVal editInstlVos As List(Of TYosanBuhinEditInsuVo))
            'ここでeditVoと照らし合わせてとるかとらないか判断させる'

            Dim instlVosByHyojijun As Dictionary(Of Integer, List(Of TYosanBuhinEditInsuVo)) = MakeInstlVosByHyojijun(editVos, editInstlVos)

            'For Each editVo As TYosanBuhinEditVo In editVos
            ...
            '        Continue For
            '    End If
            '    For Each instlVo As TYosanBuhinEditInsuVo In instlVosByHyojijun(editVo.BuhinNoHyoujiJun)
            '        'ここに条件を追加'
            '        Dim insu As BuhinKoseiInsuCellVo = InsuVo(editVo.BuhinNoHyoujiJun, instlVo.PatternHyoujiJun)
            '        VoUtil.CopyProperties(instlVo, insu)
            '        PostUpdateInsuVo(editVo.BuhinNoHyoujiJun, instlVo.PatternHyoujiJun, insu)
            '    Next
            'Next
            Dim index As Integer = 0
            For Each editVo As TYosanBuhinEditVo In editVos
                VoUtil.CopyProperties(editVo, Record(index))
                If Not instlVosByHyojijun.ContainsKey(editVo.BuhinNoHyoujiJun) Then
                    Continue For
                End If
                For Each instlVo As TYosanBuhinEditInsuVo In instlVosByHyojijun(editVo.BuhinNoHyoujiJun)
                    'ここに条件を追加'
                    Dim insu As BuhinKoseiInsuCellVo = InsuVo(index, instlVo.PatternHyoujiJun)
                    VoUtil.CopyProperties(instlVo, insu)
                    PostUpdateInsuVo(index, instlVo.PatternHyoujiJun, insu)
                Next
                index = index + 1
            Next
        End Sub
        ...
        ''' <param name="editInstlVos">試作部品編集INSTL情報</param>
        ''' <remarks></remarks>
        Public Sub New(ByVal editVos As List(Of TYosanBuhinEditVo), ByVal editInstlVos As List(Of TYosanBuhinEditInsuVo), ByVal flag As Integer)
            'ここでeditVoと照らし合わせてとるかとらないか判断させる'

            '2012/02/24'
            'For Each Vo As TShisakuBuhinEditVo In editVos
            '    'レベル０を元にCraeteColumnを作成する'
            '    Dim instlColumn As InstlColumnBag

            'Next

            Dim instlVosByHyojijun As Dictionary(Of Integer, List(Of TYosanBuhinEditInsuVo)) = MakeInstlVosByHyojijunA(editVos, editInstlVos)
            Dim No As Integer = 0
            For Each editVo As TYosanBuhinEditVo In editVos
        ...
        ''' <returns>Dictionary</returns>
        ''' <remarks></remarks>
        Private Shared Function MakeInstlVosByHyojijunA(ByVal editVos As List(Of TYosanBuhinEditVo), ByVal editInstlVos As List(Of TYosanBuhinEditInsuVo)) As Dictionary(Of Integer, List(Of TYosanBuhinEditInsuVo))
            ...
            For Each vo As TYosanBuhinEditInsuVo In editInstlVos
                If Not instlVosByHyojijun.ContainsKey(No) Then
                    instlVosByHyojijun.Add(No, New List(Of TYosanBuhinEditInsuVo))
                End If
        ...
        Public Function Copy() As BuhinKoseiMatrix
            Dim result As New BuhinKoseiMatrix()
        ...
        Public Function Copy4KoseiTenkaiTmpUse() As BuhinKoseiMatrix
            'copyだと、copy元とcopy先で同一のInstlColumnを参照されており、copy先の一時加工（削除）がcopy元にも反映してしまう。
            Dim result As New BuhinKoseiMatrix()
            'For Each colindex As Integer In Me.GetInputInsuColumnIndexes
            '    result.InstlColumn(colindex) = Me.InstlColumn(colindex)
            'Next
            ...
            Return result
        End Function
        Public Function CopyByIndex(ByVal indexList As List(Of Integer)) As BuhinKoseiMatrix
            '必要な行だけコピーする。
            Dim result As New BuhinKoseiMatrix()
            For Each colindex As Integer In Me.GetInputInsuColumnIndexesByRowIndex(indexList)
                result.InstlColumn(colindex) = Me.InstlColumn(colindex)
            Next
            Dim newrowindex As Integer = 0
            For Each rowindex As Integer In indexList
                result.Record(newrowindex) = Me.Record(rowindex)
            ...
            For Each colindex As Integer In Me.GetInputInsuColumnIndexesByRowIndex(indexList)
                newrowindex = 0
                For Each rowindex As Integer In indexList
                    result.InsuVo(newrowindex, colindex) = Me.InsuVo(rowindex, colindex)
                    result.InsuSuryo(newrowindex, colindex) = Me.InsuSuryo(rowindex, colindex)
                    newrowindex = newrowindex + 1
                Next
            Next
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Shared Function NewInnerJoinHyoujiJun(ByVal editVos As List(Of TYosanBuhinEditVo), ByVal editInstlVos As List(Of TYosanBuhinEditInsuVo)) As BuhinKoseiMatrix

            Dim instlVosByHyojijun As Dictionary(Of Integer, List(Of TYosanBuhinEditInsuVo)) = MakeInstlVosByHyojijun(editVos, editInstlVos)
            Dim extractedEditVos As New List(Of TYosanBuhinEditVo)
            For Each editVo As TYosanBuhinEditVo In editVos
                If instlVosByHyojijun.ContainsKey(editVo.BuhinNoHyoujiJun) Then
                    extractedEditVos.Add(editVo)
                End If
            Next
            Return New BuhinKoseiMatrix(extractedEditVos, editInstlVos)
        End Function
        ...
        Private Sub PostUpdateInsuVo(ByVal rowIndex As Integer, ByVal columnIndex As Integer, ByVal updatedValue As BuhinKoseiInsuCellVo)
            ...
            'INSTL列の同期
            Dim columnBag As InstlColumnBag = InstlColumn(columnIndex)
            Dim recordVo As BuhinKoseiRecordVo = Record(rowIndex)
            ...
            Else
                ...
                Dim beforeIndexRecord As BuhinKoseiRecordVo = GetBeforeIndexRecord(rowIndex, columnIndex)
                Dim index As Integer = 0
        ...
        ''' <returns>行情報</returns>
        ''' <remarks></remarks>
        Default Public Property Record(ByVal rowIndex As Integer) As BuhinKoseiRecordVo
            Get
        ...
        ''' 行情報を返す（員数以外）
        ''' </summary>
        Public ReadOnly Property Records() As ICollection(Of BuhinKoseiRecordVo)
            Get
                Dim result As New List(Of BuhinKoseiRecordVo)
        ...
        Public Property InstlColumn(ByVal columnIndex As Integer) As InstlColumnBag
            Get
                Return _columnBags(columnIndex)
            End Get
            Set(ByVal value As InstlColumnBag)
        ...
        ''' <remarks></remarks>
        Public Sub InstlColumnAdd(ByVal columnIndex As Integer, ByVal value As InstlColumnBag)
            _columnBags(columnIndex) = value
        End Sub
        ...
        ''' <remarks></remarks>
        Public Function GetBeforeIndexRecord(ByVal rowIndex As Integer, ByVal columnIndex As Integer) As BuhinKoseiRecordVo
        ...
        ''' <returns>存在しなければ、nothing</returns>
        ''' <remarks></remarks>
        Public Function GetParentRecord(ByVal rowIndex As Integer, ByVal columnIndex As Integer) As BuhinKoseiRecordVo

            Dim buhinKoseiRecordVo As BuhinKoseiRecordVo = Record(rowIndex)
            If buhinKoseiRecordVo.YosanLevel = 0 Then
                Return Nothing
            End If
            Dim indexes As List(Of Integer) = GetInputRowIndexesOnColumn(columnIndex)
            For i As Integer = 1 To indexes.Count
                ...
                If rowIndex <= index Then
                    Continue For
                End If
                If Record(index).YosanLevel + 1 = buhinKoseiRecordVo.YosanLevel Then
                    Return Record(index)
                End If
            Next
            Return Nothing
        ...
        ''' <remarks></remarks>
        Public Function GetParentRecords(ByVal rowIndex As Integer) As BuhinKoseiRecordVo()
            Dim aSet As New Dictionary(Of BuhinKoseiRecordVo, Integer)
            For Each columnIndex As Integer In GetInputInsuColumnIndexesOnRow(rowIndex)
                Dim record As BuhinKoseiRecordVo = GetParentRecord(rowIndex, columnIndex)
                If record Is Nothing OrElse aSet.ContainsKey(record) Then
                    Continue For
                End If
                aSet.Add(record, 0)
            Next
            Dim results As New List(Of BuhinKoseiRecordVo)(aSet.Keys)
        ...
        End Function
        '↓↓2014/10/10 酒井 ADD BEGIN
        Public Function GetInputInsuColumnIndexesByRowIndex(ByVal indexList As List(Of Integer)) As ICollection(Of Integer)
            Dim aList As New List(Of Integer)
            For Each rowIndex As Integer In indexList
                For Each columnIndex As Integer In GetInputInsuColumnIndexesOnRow(rowIndex)
                    If aList.Contains(columnIndex) Then
        ...
        ''' <remarks></remarks>
        Public Sub Insert(ByVal rowIndex As Integer, ByVal insertRecord As BuhinKoseiMatrix)
        ...
        ''' <remarks></remarks>
        Public Function ExtractUnderLevel(ByVal rowIndex As Integer) As BuhinKoseiMatrix
            ' 前提条件
            ' レベルだけを見て、レベル配下を抽出する
            ...
            Dim aList As New List(Of Integer)(_recordBags.Keys)
            aList.Sort()
            If Not IsNumeric(Record(rowIndex).YosanLevel) Then
            ...
            For Each index As Integer In aList
                ...
                End If
                If rowIndex < index Then
                    If Not IsNumeric(Record(index).YosanLevel) Then
                        Continue For
                    End If
                    If Record(index).YosanLevel <= Record(rowIndex).YosanLevel Then
                        Exit For
                    End If
                End If
        ...
        Public Sub CorrectLevelBy(ByVal baseLevel As Integer?)
            ...
            If baseLevel Is Nothing Then
                For Each index As Integer In GetInputRowIndexes()
                    Record(index).YosanLevel = Nothing
                Next
                Return
            End If

            Dim indexes As New List(Of Integer)(GetInputRowIndexes())
            If Not IsNumeric(Record(indexes(0)).YosanLevel) Then
                'エラーを発生させない'
                Return
                Throw New ArgumentException("基点となるレベル値が数値ではない")
            End If
            Dim diffLevel As Integer = Record(indexes(0)).YosanLevel - baseLevel
            For Each index As Integer In indexes
                If Not IsNumeric(Record(index).YosanLevel) Then
                    Continue For
                End If
                Record(index).YosanLevel -= diffLevel
            Next
        End Sub
