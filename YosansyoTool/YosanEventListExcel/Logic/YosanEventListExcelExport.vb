...
Namespace YosanEventListExcel.Logic
    Public Class YosanEventListExcelExport

        'から行
        Private dataStartRowIndex As Integer
        Private _eventCode As String
        Private _tEventVos As New List(Of TYosanEventVo)
        Private _view As FrmYosanEventListExcel
        Private _view_shukei As frmYosanEventExcelShorichu
        Private _logic As DispYosanEventListExcel
...
#Region "コンストラクタ "
        ...
        ''' <param name="f"></param>
        ''' <param name="fs"></param>
        ''' <param name="tEventVos"></param>
        ''' <remarks></remarks>
        Public Sub New(ByVal dataIni As Integer, ByVal eventCode As String, _
                       ByVal filePath As String, ByVal eventList As DispYosanEventListExcel, _
                       ByVal f As FrmYosanEventListExcel, ByVal fs As frmYosanEventExcelShorichu, _
                       Optional ByVal tEventVos As List(Of TYosanEventVo) = Nothing)

            Me._eventCode = eventCode
            Me._tEventVos = tEventVos
            Me._logic = eventList
            Me._view = f
            Me._view_shukei = fs
            ...
            If Not ShisakuComFunc.IsFileOpen(fileName) Then
                Using xls As New ShisakuExcel(fileName)
                    ...
                    'sheet2 Excel出力集計
                    SetYosanSyukeiSheet(xls, dataIni)

                    '1レコード目以降処理する。
                    If dataIni > 0 Then

                        '予算部品表選択情報を取得する。
                        Dim _BuhinhyoVos As List(Of TYosanBuhinSelectVo) = eventList.YosanBuhinhyoList(eventCode)

                        '予算部品表選択情報分の部品表を出力する。
                        For Each vo As TYosanBuhinSelectVo In _BuhinhyoVos
                            yosanUnitKbn = vo.BuhinhyoName
                            yosanBuhinEditVos = yosanEventListExcelDao.FindYosanBuhinEdit(eventCode, yosanUnitKbn)
                            yosanBuhinEditInsuVos = yosanEventListExcelDao.FindYosanBuhinEditInsu(eventCode, yosanUnitKbn)
                            '予算部品編集情報を取得できた場合
                            If 0 < yosanBuhinEditVos.Count Then
                                '予算部品編集パターン情報取得
                                yosanBuhinEditPatternVos = yosanEventListExcelDao.FindYosanBuhinEditPattern(eventCode, yosanUnitKbn)
                                '予算部品編集員数情報取得
                                instlVosByHyojijun = MakeInstlVosByHyojijun(yosanBuhinEditVos, yosanBuhinEditInsuVos)
                                'sheet3
                                setYosanBuhinSheet(eventCode, xls, yosanBuhinEditVos, instlVosByHyojijun, yosanBuhinEditPatternVos, yosanUnitKbn)
                            End If
                        Next
...
#Region "Sheet1 "
        ...
        Private Sub SetYosanEventSheet(ByVal xls As ShisakuExcel)
            ...
            ' ''余分なシートを削除する。
            xls.DeleteSheetTrialManufacture(2)
            xls.DeleteSheetTrialManufacture(2)
            'xls.DeleteSheetTrialManufacture(3)
        ...
        Private Sub setYosanEventSheetBody(ByVal xls As ShisakuExcel)
            ...
            If StringUtil.IsNotEmpty(_tEventVos) Then
                For Each Vo As TYosanEventVo In _tEventVos
...
#Region "Sheet2 "
        ...
        ''' <param name="xls">目的Excelファイル</param>
        ''' <remarks></remarks>
        Private Sub SetYosanSyukeiSheet(ByVal xls As ShisakuExcel, ByVal dataRow As Integer)

            '出力開始行を設定
            dataStartRowIndex = 2

            If dataRow = 0 Then
                If Not _tEventVos Is Nothing Then
                    If _tEventVos.Count > 1 Then
                        xls.AddSheet("集計表結果")
                    End If
                End If
                'ここではしない'
                ...
                ''造り方完成車情報出力
                'setYosanTukurikataSheetBody(xls, _view_shukei.spdTukurikataKanseisha_Sheet1)

                'dataStartRowIndex = dataStartRowIndex + _view_shukei.spdTukurikataKanseisha_Sheet1.RowCount + 1

                ''造り方ホワイトボディ情報出力
                'setYosanTukurikataSheetBody(xls, _view_shukei.spdTukurikataWBsha_Sheet1)

                'dataStartRowIndex = dataStartRowIndex + _view_shukei.spdTukurikataWBsha_Sheet1.RowCount + 1
            Else
                xls.AddSheet("集計表" & _eventCode)
...
#Region "集計値spread出力 "
        ...
        Private Sub setYosanZaimuJisekiBody(ByVal xls As ShisakuExcel)
            Dim sheet As SheetView = _view_shukei.spdZaimuJiseki_Sheet1
...
#Region "製作台数spread出力 "
        ...
        Private Sub setYosanDaisuSheetBody(ByVal xls As ShisakuExcel)
            Dim sheet As SheetView = _view_shukei.spdSeisakuDaisu_Sheet1
...
#Region "造り方spread出力 "
        ...
        Private Sub setYosanTukurikataTitleRow(ByVal xls As ShisakuExcel, ByVal sheet As SheetView)
            ...
            If sheet.Equals(_view_shukei.spdTukurikataKanseisha_Sheet1) Then
...
#Region "金材spread出力 "
        ...
        Public Sub setYosanKanazaiSheetBody(ByVal xls As ShisakuExcel)
            Dim sheet As SheetView = _view_shukei.spdKanazai_Sheet1
...
#Region "Sheet部品表名 "
        ...
        ''' <remarks></remarks>
        Private Sub setYosanBuhinSheet(ByVal eventCode As String, ByVal xls As ShisakuExcel, _
            ...
            'Sheet名を設定
            xls.AddSheet("部品表_" & eventCode & "_" & yosanUnitKbn & "部品表")
            '出力 部品表各カラム名に数値を設定
            SetYosanBuhinColumnNo(yosanBuhinEditPatternVos)
            '出力 部品表情報を設定
            setYosanBuhinSheetBody(xls, yosanBuhinEditVos, instlVosByHyojijun, yosanBuhinEditPatternVos)
...
#Region "追加"
        ...
        ''' <param name="tEventVos"></param>
        ''' <remarks></remarks>
        Public Sub New(ByVal dataIniaaa As Integer, _
                       ByVal dataIni As Integer, _
                       ByVal eventCode As String, _
                       ByVal filePath As String, _
                       ByVal eventList As DispYosanEventListExcel, _
                       ByVal f As FrmYosanEventListExcel, _
                       ByVal fs As frmYosanEventExcelShorichu, _
                       Optional ByVal tEventVos As List(Of TYosanEventVo) = Nothing)
...
#Region "実験"
        ...
        ''' <summary>
        ''' スプレッドシートに今回エクセルで用意した内容を全部乗っけてみる。
        ''' </summary>
        ''' <remarks></remarks>
        Private Sub TestSheet(ByVal xls As ShisakuExcel)
