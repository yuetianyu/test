...
#Region " ビュー初期化 "
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function InitView(ByVal kaihatsuFugo As String, ByVal selectKubun As String) As RESULT
            ...
            SetSpdDataField()
            ''spreadにデータを設定する
            SetSpreadSource(kaihatsuFugo, selectKubun)
...
#Region "SPREADで 列のセルの水平方向の配置を設定する。"
        Public Sub SetSpdColPro()
            'ラベル名と横幅変更
            m_spCom.GetColFromTag(TAG_KUBUN_NAME).Label = "区分名称"
            m_spCom.GetColFromTag(TAG_KUBUN_NAME).Width = 230
            ...
            For i As Integer = 0 To m_view.spdKubun_Sheet1.ColumnCount - 1
                m_view.spdKubun_Sheet1.Columns(i).Resizable = False
            Next
...
#Region " spreadでデータを取得する "
        Public Function GetSpdDaiKubunList(ByVal kaihatsuFugo As String) As DataTable
            ...
            Return dtData
        End Function
        Public Function GetSpdKubunList(ByVal kaihatsuFugo As String) As DataTable
            Dim dtData As New DataTable
            Using db As New SqlAccess(g_kanrihyoIni(DB_KEY_EBOM))
                db.Open()
...
#Region "spreadのbind"
        Private Sub SetSpreadSource(ByVal kaihatsuFugo As String, ByVal selectKubun As String)
            ...
            If StringUtil.Equals(selectKubun, HOYOU_SELECT_EBOM_SYSTEM_DAI) Then
                'SPREADのデータソースを設定する
                m_view.spdKubun_Sheet1.DataSource = GetSpdDaiKubunList(kaihatsuFugo)
            Else
                'SPREADのデータソースを設定する
                m_view.spdKubun_Sheet1.DataSource = GetSpdKubunList(kaihatsuFugo)
            End If
