...
#Region " ビュー初期化 "
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function InitView(ByVal kaihatsuFugo As String, ByVal shisakuEvent As String, ByVal shisakuEventName As String, ByVal strKuke As String, ByVal strChkKuke As Boolean, ByVal strSelect As String) As RESULT
            ...
            SetSpdDataField()
            ''spreadにデータを設定する
            SetSpreadSource(kaihatsuFugo, shisakuEvent, shisakuEventName, strKuke, strChkKuke, strSelect)
...
#Region " spreadでデータを取得する "
        Public Function GetIchiranList(ByVal kaihatsuFugo As String, ByVal strKuke As String, ByVal strChkKuke As Boolean) As DataTable
            Dim dtData As New DataTable
            Using db As New SqlAccess(g_kanrihyoIni(DB_KEY_EBOM))
                db.Open()
                db.Fill(GetBuhinSakuseiList(kaihatsuFugo, strKuke, strChkKuke), dtData)
            End Using
            Return dtData
        End Function
        Public Function GetIchiranShisakuListFromC(ByVal kaihatsuFugo As String, ByVal shisakuEventCode As String, ByVal strSelect As String, ByVal strKuke As String, ByVal strChkKuke As Boolean) As DataTable
            Dim dtData As New DataTable
            Using db As New SqlAccess(g_kanrihyoIni(DB_KEY_EBOM))
                db.Open()
                db.Fill(GetBuhinSakuseiShisakuListFromC(kaihatsuFugo, shisakuEventCode, strSelect, strKuke, strChkKuke), dtData)
            End Using
            Return dtData
        End Function
#End Region
...
#Region "spreadのbind"
        Private Sub SetSpreadSource(ByVal kaihatsuFugo As String, _
                                    ...
                                    ByVal shisakuEventName As String, ByVal strKuke As String, ByVal strChkKuke As Boolean, ByVal strSelect As String)
            If StringUtil.IsEmpty(shisakuEventCode) And StringUtil.IsEmpty(shisakuEventName) Then
                'SPREADのデータソースを設定する
                m_view.spdBlock_Sheet1.DataSource = GetIchiranList(kaihatsuFugo, strKuke, strChkKuke)
            ElseIf StringUtil.IsNotEmpty(shisakuEventCode) Then
                'SPREADのデータソースを設定する
                m_view.spdBlock_Sheet1.DataSource = GetIchiranShisakuListFromC(kaihatsuFugo, shisakuEventCode, strSelect, strKuke, strChkKuke)
            End If
...
#Region "ブロック一覧"
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Shared Function GetBuhinSakuseiList(ByVal kaihatsuFugo As String, ByVal strKuke As String, ByVal strChkKuke As Boolean) As String
            ...
            With sql
                ...
                If strKuke.Equals("M") OrElse strKuke.Equals("T") Then
                    If strChkKuke = False Then
                        .AppendLine("    AND T008.MT_KBN = '" & strKuke & "' ")
                    ElseIf strChkKuke = True Then
                        .AppendLine("    AND ( T008.MT_KBN = '" & strKuke & "' " + " OR T008.MT_KBN = '' ) ")
                    End If
                End If
        ...
        Public Shared Function GetBuhinSakuseiShisakuListFromC(ByVal kaihatsuFugo As String, _
                                                               ByVal shisakuEventCode As String, ByVal strSelect As String, _
                                                               ByVal strKuke As String, ByVal strChkKuke As Boolean) As String
            Dim sql As New System.Text.StringBuilder()
            With sql
                .Remove(0, .Length)
                'strSelectによって参照テーブルを変更
                '   新試作手配システムのブロック情報から
                If StringUtil.Equals(strSelect, HOYOU_SELECT_VER1) = False And StringUtil.Equals(strSelect, HOYOU_SELECT_VER2) = False Then
                    .AppendLine("SELECT  ")
                    .AppendLine("    T004.BLOCK_NO AS BLOCK_NO, ")
                    .AppendLine("    T004.BLOCK_NAME AS BLOCK_NAME ")
                    .AppendLine("FROM ")
                    ...
                    .AppendLine("    INNER JOIN " & RHACLIBF_DB_NAME & ".dbo.RHAC0080 AS T008 ON INSTL.SHISAKU_BLOCK_NO = T008.BLOCK_NO_KINO ")
                    .AppendLine("WHERE ")
                    ...
                    .AppendLine("ORDER BY ")
                    .AppendLine("   T004.BLOCK_NO ,T004.BLOCK_NAME ")
                ElseIf StringUtil.Equals(strSelect, HOYOU_SELECT_VER1) = True Then
                    .AppendLine("SELECT  ")
                    .AppendLine("    T004.BLOCK_NO AS BLOCK_NO, ")
                    .AppendLine("    T004.BLOCK_NAME AS BLOCK_NAME ")
                    .AppendLine("FROM ")
                    .AppendLine(MBOM_DB_NAME & ".dbo.T_GENCHO_SEKKEI_BLOCK AS INSTL ")
                    .AppendLine("    INNER JOIN " & MBOM_DB_NAME & ".dbo.T_GENCHO_EVENT AS EVENT ON INSTL.GENCHO_EVENT_CODE = EVENT.GENCHO_EVENT_CODE ")
                    .AppendLine("    INNER JOIN " & RHACLIBF_DB_NAME & ".dbo.RHAC0040 AS T004 ON INSTL.GENCHO_BLOCK_NO = T004.BLOCK_NO ")
                    .AppendLine("    INNER JOIN " & RHACLIBF_DB_NAME & ".dbo.RHAC0080 AS T008 ON INSTL.GENCHO_BLOCK_NO = T008.BLOCK_NO_KINO ")
                    .AppendLine("WHERE ")
                    ...
                    .AppendLine("ORDER BY ")
                    .AppendLine("   T004.BLOCK_NO ,T004.BLOCK_NAME ")
                ElseIf StringUtil.Equals(strSelect, HOYOU_SELECT_VER2) = True Then
                    .AppendLine("SELECT  ")
                    .AppendLine("    T004.BLOCK_NO AS BLOCK_NO, ")
                    .AppendLine("    T004.BLOCK_NAME AS BLOCK_NAME ")
                    .AppendLine("FROM ")
                    .AppendLine(MBOM_DB_NAME & ".dbo.T_FUNC_SEKKEI_BLOCK AS INSTL ")
                    .AppendLine("    INNER JOIN " & MBOM_DB_NAME & ".dbo.T_FUNC_EVENT AS EVENT ON INSTL.GENCHO_EVENT_CODE = EVENT.GENCHO_EVENT_CODE ")
                    .AppendLine("    INNER JOIN " & RHACLIBF_DB_NAME & ".dbo.RHAC0040 AS T004 ON INSTL.GENCHO_BLOCK_NO = T004.BLOCK_NO ")
                    .AppendLine("    INNER JOIN " & RHACLIBF_DB_NAME & ".dbo.RHAC0080 AS T008 ON INSTL.GENCHO_BLOCK_NO = T008.BLOCK_NO_KINO ")
                    .AppendLine("WHERE ")
                    .AppendLine("    EVENT.GENCHO_KAIHATSU_FUGO = '" & kaihatsuFugo & "' ")
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Shared Function GetBuhinSakuseiShisakuListFromN(ByVal kaihatsuFugo As String, ByVal shisakuEventName As String, ByVal strSelect As String) As String
            Dim sql As New System.Text.StringBuilder()
            With sql
                .Remove(0, .Length)
                .AppendLine("SELECT  ")
