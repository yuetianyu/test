...
Namespace ShisakuBuhinEdit.Kosei.Logic.Matrix
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Class HoyouBuhinMakeStructureResultImpl
        Implements HoyouBuhinMakeStructureResult

        Private ReadOnly editDao As THoyouBuhinEditDao
        Private ReadOnly editInstlDao As TShisakuBuhinEditInstlDao
        Private ReadOnly makeDao As HoyouBuhinMakeStructureResultDao
        ...
        Public Sub New(ByVal shisakuEventCode As String, ByVal shisakuBukaCode As String, ByVal shisakuBlockNo As String)
            Me.New(New THoyouBuhinEditDaoImpl, New TShisakuBuhinEditInstlDaoImpl, New HoyouBuhinMakeStructureResultDaoImpl)
        ...
        ''' <remarks></remarks>
        Public Sub New(ByVal editDao As THoyouBuhinEditDao, ByVal editInstlDao As TShisakuBuhinEditInstlDao, ByVal makeDao As HoyouBuhinMakeStructureResultDao)
            Me.editDao = editDao
        ...
        '構成再展開から開発符号をセットする'
        Public Sub KaihatsuFugoSet(ByVal KaihatsuFugo As String) Implements HoyouBuhinMakeStructureResult.KaihatsuFugoSet
        ...
        '''' <returns>部品表</returns>
        '''' <remarks></remarks>
        'Public Function Compute(ByVal aStructureResult As HoyouBuhinStructureResult, _
        '                        ByVal JikyuUmu As String, _
        '                        ByVal kaihatsuFugo As String) As HoyouBuhinBuhinKoseiMatrix Implements HoyouBuhinMakeStructureResult.Compute
        '    Return Compute(aStructureResult, a0553flag, 0, kaihatsuFugo)
        'End Function
        ''' <summary>
        ''' 「構成の情報」を元に部品表を作成する
        ''' </summary>
        ...
        Public Function Compute(ByVal aStructureResult As HoyouBuhinStructureResult, _
            ...
            If Not aStructureResult.IsExist Then
                Return New HoyouBuhinBuhinKoseiMatrix
            End If

            oriBuhinNo = aStructureResult.OriginalBuhinNo
            bfBuhinNo = aStructureResult.OriginalBuhinNo

            '色コード付きが存在する場合があるので##に置き換える'
            '2012/02/22 色コードつきで存在する構成も存在するのでコメントアウト'
            'If aStructureResult.BuhinNo.Length = 12 Then
            '    If Not StringUtil.Equals(Right(aStructureResult.BuhinNo, 2), "##") Then
            '        Dim color As String = "##"
            '        Dim buhinNo As String = Left(aStructureResult.BuhinNo, 10) + color
            '        aStructureResult.BuhinNo = buhinNo
            '    End If
            'End If
            ...
            If Not aStructureResult.IsEBom Then
                ...
                If aStructureResult.EditVo IsNot Nothing Then
                    ...
                    '自給品を含めて取得する。
                    Dim editVos As List(Of THoyouBuhinEditVo) = FindEditBy(editInstlVo)

                    Dim result As HoyouBuhinBuhinKoseiMatrix = HoyouBuhinBuhinKoseiMatrix.NewInnerJoinHyoujiJun(editVos, editInstlVos)
                ...
                ElseIf aStructureResult.InstlVo IsNot Nothing Then
                    ...
                    '自給品を含めて取得する。
                    Dim editVos As List(Of THoyouBuhinEditVo) = FindEditBy(aStructureResult.InstlVo)
            ...
            If Rhac0552Vo Is Nothing Then
                ...
                If _a0553flag = 0 Then
                    ...
                    If StringUtil.IsEmpty(kaihatsuFugo) Then
                        ...
                        'eventVoの値が格納されない場合エラーとなるので eventVoの状態判定を追加
                ...
                '20141218 TSUNODA ADD
                '既存判定（開発符号がある場合）と今回の開発符号が無い場合で処理を分離した
                If kaihatsuFugo.Trim <> "" Then
                    checkVo = makeDao.FindByBuhinRhac0553(kaihatsuFugo, inputedBuhinNo)
                Else
                    '20141218 このメソッドを増設した
                    checkVo = makeDao.FindByBuhinRhac0553NowBuhinOnly(inputedBuhinNo)
                End If

                If Not checkVo Is Nothing Then

                    'クエリ結果が得られた場合でも、上からの開発符号無し状態の処理があるため取得したクエリ結果から開発符号を補完する対応を織り込んだ
                    If kaihatsuFugo.Trim <> "" Then
                        Return BuhinKousei(inputedBuhinNo, aStructureResult, baseLevel, kaihatsuFugo)
                    Else
                        '20141218 TSUNODA ADD
                        '開発符号が無い場合は、取得したクエリから開発符号を補完して構成情報の取得を行う
                        Return BuhinKousei(inputedBuhinNo, aStructureResult, baseLevel, checkVo.KaihatsuFugo)
                    End If
                Else
                    '553にも無ければ551を見に行く'
            ...
            Dim newKoseiMatrix As New HoyouBuhinBuhinKoseiMatrix
            ...
            Dim mergeNode As New HoyouBuhinMergeNodeList(Of Rhac0552Vo, Rhac0532Vo)(New RhacNodeAccessor(New MakerNameResolverImpl(rhac0610Vos, inputedBuhinNoVo)))

            mergeNode.Compute(singleMatrices(0).Nodes)

            newKoseiMatrix = mergeNode.ResultMatrix
            'Dim newKoseiMatrix As HoyouBuhinBuhinKoseiMatrix = mergeNode.ResultMatrix
            If Not isInstlHinban Then
                newKoseiMatrix.RemoveRow(0) ' parentVoForBuhinNo.BuhinNoOya が先頭にいるから削除しておく
            End If

            ''集計コードがＲの子部品の構成情報を除外する。"R"(下で「集計コード」再設定を追加のため)
            ...
            If Not newKoseiMatrix Is Nothing Then
                For Each index As Integer In newKoseiMatrix.GetInputRowIndexes
                    ...
                    If Not StringUtil.IsEmpty(newKoseiMatrix(index).BuhinNo) Then
                        ...
                        '集計コードを再設定する'"R"
                        If (newKoseiMatrix(index).ShukeiCode = "R") _
                        Or (newKoseiMatrix(index).SiaShukeiCode = "R") Then
                            ...
                            vo = impl.FindByBuhinShukei(newKoseiMatrix(index).BuhinNo)
                            If Not vo Is Nothing Then
                                If Not StringUtil.IsEmpty(vo.ShukeiCode) _
                                Or Not StringUtil.IsEmpty(vo.SiaShukeiCode) Then
                                    'If (newKoseiMatrix(index).ShukeiCode = "R") Then
                                    newKoseiMatrix(index).ShukeiCode = vo.ShukeiCode
                                    'End If
                                    'If (newKoseiMatrix(index).SiaShukeiCode = "R") Then
                                    newKoseiMatrix(index).SiaShukeiCode = vo.SiaShukeiCode
                                    'End If
                                End If
                            End If
                        End If
                        ...
                        If StringUtil.IsEmpty(newKoseiMatrix(index).ShukeiCode) Then
                            ...
                            ElseIf StringUtil.Equals(newKoseiMatrix(index).SiaShukeiCode, "A") Then
                                ...
                                'newKoseiMatrix(index).KyoukuSection = "9SX00"
        ...
        ''' <returns>部品表</returns>
        ''' <remarks></remarks>
        Public Function Compute2(ByVal aStructureResult As HoyouBuhinStructureResult, ByVal a0553flag As Integer, ByVal baseLevel As Integer?) As HoyouBuhinBuhinKoseiMatrix Implements HoyouBuhinMakeStructureResult.Compute2
            ...
            If Not aStructureResult.IsEBom Then
                ...
                ElseIf aStructureResult.InstlVo IsNot Nothing Then
                    ...
                    If result.Records.Count = 0 Then
                        ...
                        Dim editVos2 As New List(Of THoyouBuhinEditVo)
                        Dim dummyEditVos2 As New THoyouBuhinEditVo
                        ...
                        Dim result2 As HoyouBuhinBuhinKoseiMatrix = HoyouBuhinBuhinKoseiMatrix.NewInnerJoinHyoujiJun(editVos2, editInstlVos2)
        ...
        '''' <returns>部品表</returns>
        '''' <remarks></remarks>
        'Public Function ComputeKosei(ByVal aStructureResult As HoyouBuhinStructureResult, ByVal a0553Flag As Integer, ByVal baseLevel As Integer?) As HoyouBuhinBuhinKoseiMatrix Implements HoyouBuhinMakeStructureResult.ComputeKosei
        '    Dim target As String
        '    Dim KaihatsuFugo As String
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function GetBuhinKosei(ByVal aStructureResult As HoyouBuhinStructureResult, ByVal a0553Flag As Integer) As HoyouBuhinBuhinKoseiMatrix Implements HoyouBuhinMakeStructureResult.GetBuhinKosei
            Dim target As String
            Dim KaihatsuFugo As String
            If Left(aStructureResult.YobidashiMoto, 4) = "0533" Then
                target = "0533"
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Private Function SetKyoukuSection0552(ByVal buhinNo As String, ByVal originalBuhinNo As String, ByVal rhac0552Vos As List(Of Rhac0552Vo), ByVal KoseiMatrix As HoyouBuhinBuhinKoseiMatrix) As HoyouBuhinBuhinKoseiMatrix
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Private Function SortMatrix(ByVal koseiMatrix As HoyouBuhinBuhinKoseiMatrix) As HoyouBuhinBuhinKoseiMatrix
            'レベル０は対象外'
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Private Function MergeMatrix(ByVal koseiMatrix As HoyouBuhinBuhinKoseiMatrix) As HoyouBuhinBuhinKoseiMatrix
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Private Function GetLastLevel(ByVal koseiMatrix As HoyouBuhinBuhinKoseiMatrix) As Integer
        ...
        ''' <remarks></remarks>
        Private Function FindMinInstlHinbanHyoujiJunEditInstlBy(ByVal editVo As THoyouBuhinEditVo) As TShisakuBuhinEditInstlVo

            Dim param As New TShisakuBuhinEditInstlVo
            param.ShisakuEventCode = editVo.HoyouEventCode
            param.ShisakuBukaCode = editVo.HoyouBukaCode
            param.ShisakuBlockNo = editVo.HoyouTanto
            param.ShisakuBlockNoKaiteiNo = editVo.HoyouTantoKaiteiNo
        ...
        Private Function FindEditInstlByInstlHinbanHyoujiJun(ByVal blockInstlVo As TShisakuSekkeiBlockInstlVo) As List(Of TShisakuBuhinEditInstlVo)
            'Dim param As New TShisakuBuhinEditInstlVo
            ...
            result = makeDao.FindbyEditInstl(blockInstlVo.ShisakuEventCode, blockInstlVo.InstlHinban)
            Return result
        End Function

        Private Function FindEditBy(ByVal editInstlVo As TShisakuBuhinEditInstlVo) As List(Of THoyouBuhinEditVo)
            Dim param As New THoyouBuhinEditVo
            param.HoyouEventCode = editInstlVo.ShisakuEventCode
            param.HoyouBukaCode = editInstlVo.ShisakuBukaCode
            param.HoyouTanto = editInstlVo.ShisakuBlockNo
            param.HoyouTantoKaiteiNo = editInstlVo.ShisakuBlockNoKaiteiNo
            Return editDao.FindBy(param)
        End Function

        Private Function FindEditBy(ByVal blockInstlVo As TShisakuSekkeiBlockInstlVo) As List(Of THoyouBuhinEditVo)
            ...
            'param.ShisakuBlockNoKaiteiNo = blockInstlVo.ShisakuBlockNoKaiteiNo
            'Return editDao.FindBy(param)
            Dim result As New List(Of THoyouBuhinEditVo)
            result = makeDao.FindbyEdit(blockInstlVo.ShisakuEventCode, blockInstlVo.InstlHinban)
            Return result
        End Function
...
#Region "FM5以降～"
        ...
        ''' <returns>部品表</returns>
        ''' <remarks></remarks>
        Private Function BuhinKousei(ByVal inputedBuhinNo As String, ByVal aStructureResult As HoyouBuhinStructureResult, ByVal baseLevel As Integer, ByVal kaihatsuFugo As String) As HoyouBuhinBuhinKoseiMatrix
            '0553注記：設計展開時はベース車情報の開発符号を使用'
            '構成再展開、最新化、構成呼び出しは画面側から取得した開発符号を使用'
            '子部品展開はイベント情報の開発符号を使用する'
            ...
            'Dim mergeNode As New HoyouBuhinMergeNodeList(Of Rhac0553Vo, Rhac0533Vo)(New RhacNodeAccessor(New MakerNameResolverImplRhac0553(rhac0610Vos, inputedBuhinNoVo)))
            Dim mergeNode As New HoyouBuhinMergeNodeList(Of Rhac0553Vo, Rhac0533Vo)(New Rhac0553NodeAccessor(New MakerNameResolverImplRhac0553(Nothing, inputedBuhinNoVo)))
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Private Function SetKyoukuSection0553(ByVal buhinNo As String, ByVal originalBuhinNo As String, ByVal rhac0553Vos As List(Of Rhac0553Vo), ByVal KoseiMatrix As HoyouBuhinBuhinKoseiMatrix) As HoyouBuhinBuhinKoseiMatrix
...
#Region "パンダ前"
        ...
        ''' <returns>部品表</returns>
        ''' <remarks></remarks>
        Private Function BuhinKouseiRhac0551(ByVal inputedBuhinNo As String, ByVal aStructureResult As HoyouBuhinStructureResult, ByVal baseLevel As Integer) As HoyouBuhinBuhinKoseiMatrix
            ...
            Dim mergeNode As New HoyouBuhinMergeNodeList(Of Rhac0551Vo, Rhac0530Vo)(New Rhac0551NodeAccessor(New MakerNameResolverImplRhac0551(Nothing, inputedBuhinNoVo)))
            'Dim mergeNode As New HoyouBuhinMergeNodeList(Of Rhac0551Vo, Rhac0530Vo)(New Rhac0553NodeAccessor(New MakerNameResolverImplRhac0553(Nothing, inputedBuhinNoVo)))
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Private Function SetKyoukuSection0551(ByVal buhinNo As String, ByVal originalBuhinNo As String, ByVal rhac0551Vos As List(Of Rhac0551Vo), ByVal KoseiMatrix As HoyouBuhinBuhinKoseiMatrix) As HoyouBuhinBuhinKoseiMatrix
