...
Namespace TehaichoEdit.Excel
    Public Class BuhinExportShinchotatsu
        ...
        ''' <remarks></remarks>
        Public Sub New(ByVal eventCode As String, ByVal listCode As String, ByVal kaiteiNo As String, ByVal sw As Integer, _
                       Optional ByVal shisakuGousyaGroup As String = "")
            ...
            Using sfd As New SaveFileDialog()
                ...
                '[Excel出力系 L M]
                If sw = 0 Then
                    fileName = aEventVo.ShisakuKaihatsuFugo + aList.ShisakuEventName + shisakuGousyaGroup + Now.ToString("MMdd") + Now.ToString("HHmm") + "新調達への転送.xls"
                Else
                    fileName = aEventVo.ShisakuKaihatsuFugo + aList.ShisakuEventName + shisakuGousyaGroup + Now.ToString("MMdd") + Now.ToString("HHmm") + "新調達への転送履歴.xls"
                End If
            ...
            '-------------------------------------------------------------------------------------------
            If StringUtil.IsEmpty(shisakuGousyaGroup) Then
                aKihonListVo = impl.FindByTehaiKihonKaiteiNo(eventCode, listCode, kaiteiNo, aList.UnitKbn)
                aGousyaListVo = impl.FindByTehaiGousyaKaiteiNo(eventCode, listCode, kaiteiNo)
                aBaseListVo = impl.FindByBase(eventCode, listCode)
            Else
                aKihonListVo = impl.FindByTehaiKihonKaiteiNoToGroup(eventCode, listCode, kaiteiNo, aList.UnitKbn, shisakuGousyaGroup)
                aGousyaListVo = impl.FindByTehaiGousyaKaiteiNoToGroup(eventCode, listCode, kaiteiNo, shisakuGousyaGroup)
                aBaseListVo = impl.FindByBaseToGroup(eventCode, listCode, shisakuGousyaGroup)
            End If
            Dim i As Integer = 0
        ...
        Public Sub setSheetHeard(ByVal xls As ShisakuExcel, _
            ...
            '抽出日時'
            xls.SetValue(6, 2, "抽出日時: " + DateTime.Now)
