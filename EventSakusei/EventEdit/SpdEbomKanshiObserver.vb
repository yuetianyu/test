...
Namespace EventEdit
    ''' <summary>
    ''' ''↓↓2014/08/05 Ⅰ.5.EBOM差分出力 l) (TES)施 ADD BEGIN
    ''' </summary>
    ''' <remarks></remarks>
    Public Class SpdEbomKanshiObserver : Implements Frm9SpdObserver, Frm9SpdSetter(Of EventEditEbomKanshi)
#Region "各列のTAG"
...
#End Region
        ...
        Private Shared ReadOnly DEFAULT_LOCKED_BASE_TAGS As String() = {TAG_SEKKEI_TENKAI, TAG_KAIHATSU_FUGO, _
        ...
        Private subject As EventEditEbomKanshi
        Private ReadOnly titleRows As Integer
        Private validator As SpreadValidator
        '号車DELETEﾁｪｯｸ用に追加
        Private GousyaDeleteCheck As String = ""

        Public Sub New(ByVal spread As FpSpread, ByVal subject As EventEditEbomKanshi)
            Me.spread = spread
        ...
        Public Sub SupersedeSubject(ByVal subject As EventEditEbomKanshi) Implements Frm9SpdSetter(Of EventEditEbomKanshi).SupersedeSubject
            Me.subject = subject
        ...
        Public Sub Initialize() Implements Frm9SpdObserver.Initialize
            ...
            sheet.Columns(TAG_BASE_EVENT_CODE).Visible = False
            sheet.Columns(TAG_BASE_GOSHA).Visible = False
            sheet.Columns(TAG_SEISAKU_SYATAI_NO).Visible = False

            ''↓↓2014/08/05 Ⅰ.5.EBOM差分出力 an) (TES)施 ADD BEGIN
            sheet.Columns(TAG_SHUBETSU).Locked = True
            sheet.Columns(TAG_GOSHA).Locked = True
            sheet.Columns(TAG_BASE_EVENT_CODE).Locked = True
            sheet.Columns(TAG_BASE_GOSHA).Locked = True
            sheet.Columns(TAG_SEISAKU_SYATAI_NO).Locked = True
          
            ''↑↑2014/08/05 Ⅰ.5.EBOM差分出力 an) (TES)施 ADD END																																																																																			
        ...
        Public Sub referenceCarUnVisible()
            ...
            'ラベルを変更
            ''↓↓2014/08/05 Ⅰ.5.EBOM差分出力 ao) (TES)施 `修正 BEGIN
            'sheet.ColumnHeader.Cells.Get(0, sheet.Columns(TAG_SHIYO_JOHO_NO).Index).Value = ""

            ''↑↑2014/08/05 Ⅰ.5.EBOM差分出力 ao) (TES)施　修正 END	
        ...
        Private Sub LockSheetIfViewerChange()
            ...
            If Not subject.IsViewerMode.Equals(backIsViewer) Then
                ...
                Else
                    ...
                    ''↓↓2014/08/20 Ⅰ.5.EBOM差分出力 an) 酒井 ADD BEGIN
                    sheet.Columns(TAG_SHUBETSU).Locked = True
                    sheet.Columns(TAG_GOSHA).Locked = True
                    sheet.Columns(TAG_BASE_EVENT_CODE).Locked = True
                    sheet.Columns(TAG_BASE_GOSHA).Locked = True
                    sheet.Columns(TAG_SEISAKU_SYATAI_NO).Locked = True
                    ''↑↑2014/08/20 Ⅰ.5.EBOM差分出力 an) 酒井 ADD END		
                End If
            End If
        End Sub
            ...
            For Each rowNo As Integer In subject.GetInputRowNos
                ...
                If Not StringUtil.IsEmpty(sheet.Cells(row, sheet.Columns(TAG_GOSHA).Index).Value) Then
                    ...
                    ElseIf ErrCheckBase01 = "" And ErrCheckBase02 = "" And ErrCheckEvent01 = "OK" And ErrCheckEvent02 = "" Then
                        ...
                        '1 ベース部品表作成　機能増強 2014/07/07 ※削除（コメント）にする。
