...
Namespace ShisakuBuhinMenu.Logic.Impl.Tenkai
    ...
    Public Class BuhinEditBaseSupplier
        ...
        ''↓↓2014/07/23 Ⅰ.2.管理項目追加_am) (TES)張 ADD BEGIN
        Private TsukurikataTenkaiFlg As String
        ''↑↑2014/07/23 Ⅰ.2.管理項目追加_am) (TES)張 ADD END
...
#End Region
        ...
        ''' <param name="eventVo">試作イベント情報</param>
        ''' <remarks></remarks>
        ...
        Public Sub New(ByVal blockVo As TShisakuSekkeiBlockVo, ByVal login As LoginInfo, ByVal aShisakuDate As ShisakuDate, _
                       ...
                       ByVal sbiDao As ShisakuSekkeiBlockInstlDaoImpl, _
                       ByVal eventVo As TShisakuEventVo)
            ...
            Me.JikyuUmu = eventVo.JikyuUmu

            TsukurikataFlg = eventVo.TsukurikataFlg
            TsukurikataTenkaiFlg = eventVo.TsukurikataTenkaiFlg
            ...
            If 0 < editVos.Count Then
                ...
                Dim EIVos As List(Of TShisakuBuhinEditInstlVo)
                Dim EIVos2 As New List(Of TShisakuBuhinEditInstlVo)
                Dim BEVos As List(Of TShisakuBuhinEditVo)
                ...
                '   2012/08/22 部課コード対策
                '   部課コードをキーから外してみる。
                Dim sekkeiBlockInstlVos As List(Of TShisakuSekkeiBlockInstlVoEx) = bimpl.FindBySekkeiBlockInstl(blockVo.ShisakuEventCode, blockVo.ShisakuBlockNo)
                ...
                '2014/11/21 変更 別イベント（複数）からデータを作成した場合、元のInstlHinbanHyoujiJunでは被ってしまうため新規に発番
                Dim instlHinbanHyoujiJunList As New Hashtable
                For Each sbvo As TShisakuSekkeiBlockInstlVoEx In sekkeiBlockInstlVos
                    Dim key As New System.Text.StringBuilder
                    key.AppendLine(sbvo.InstlHinban)

                    ''2015/08/04
                    key.AppendLine(sbvo.InstlHinbanKbn)

                    key.AppendLine(sbvo.InstlDataKbn2)

                    key.AppendLine(StringUtil.Nvl(sbvo.BaseInstlHinbanHyoujiJun, ""))


                    If Not instlHinbanHyoujiJunList.Contains(key.ToString) Then
                        instlHinbanHyoujiJunList.Add(key.ToString, instlHinbanHyoujiJun)
                        instlHinbanHyoujiJun += 1
                    End If
                Next
                ...
                Dim bkInstlHinbanHyojijun As Integer = 0
                ...
                For Each sbvo As TShisakuSekkeiBlockInstlVoEx In sekkeiBlockInstlVos
                    instlFlag = False
                    If sbvo.ShisakuEventCode IsNot Nothing Then
                        If StringUtil.Equals(sbvo.InstlHinban, instlHinban) Then
                            '同一'
                            If StringUtil.Equals(sbvo.InstlHinbanKbn, instlHinbanKbn) Then
                                If StringUtil.Equals(sbvo.InstlDataKbn2, instlDataKbn) Then
                                    If StringUtil.Equals(sbvo.BaseInstlFlg2, BaseInstlFlg) Then
                                        If sbvo.OldInstlHinbanHyoujiJun = bkInstlHinbanHyojijun Then
                                            instlFlag = False
                                        Else
                                            instlFlag = True
                                        End If
                    ...
                    If instlFlag Then
                        ...
                        EIVos = bimpl.FindByBuhinEditInstl2(blockVo.ShisakuEventCode, sbvo.ShisakuEventCode, _
                                                                sbvo.ShisakuBlockNo, _
                                                                sbvo.InstlHinban, _
                                                                sbvo.InstlHinbanKbn, _
                                                                sbvo.InstlDataKbn, _
                                                                sbvo.OldInstlHinbanHyoujiJun, _
                                                                sbvo.BaseInstlFlg)

                        BEVos = bimpl.FindByBuhinEdit2(blockVo.ShisakuEventCode, sbvo.ShisakuEventCode, _
                                                       sbvo.ShisakuBlockNo, _
                                                       sbvo.InstlHinban, _
                                                       sbvo.InstlHinbanKbn, _
                                                       sbvo.InstlDataKbn, _
                        ...
                        If EIVos.Count > 0 Then
                            ...
                            For Each eivo As TShisakuBuhinEditInstlVo In EIVos
                                '2014/11/21 変更
                                'eivo.InstlHinbanHyoujiJun = sbvo.InstlHinbanHyoujiJun
                                Dim key As New System.Text.StringBuilder
                                key.AppendLine(sbvo.InstlHinban)
                                ...
                                eivo.InstlHinbanHyoujiJun = instlHinbanHyoujiJunList.Item(key.ToString)
                            ...
                            For Each bevo As TShisakuBuhinEditVo In BEVos
                                ...
                                '部品番号表示順を０から振りなおす'
                                bevo.BaseBuhinFlg = "1"
                                bevo.BaseBuhinSeq = BuhinNoHyoujiJun2
                    ...
                    instlDataKbn = sbvo.InstlDataKbn2
                    bkInstlHinbanHyojijun = sbvo.OldInstlHinbanHyoujiJun
                    BaseInstlFlg = sbvo.BaseInstlFlg2
                Next
        ...
        ''' </summary>
        ''' <remarks></remarks>
        Public Sub PerformInitialized(Optional ByVal yakanFlg As Boolean = False)
            '部課コードが空の情報が存在する'
            ...
            '_koseiMatrixはBukaCode指定なしに、試作イベントから抽出されている
            '→EBOMから取得するはずのBlockNo/BukaCodeのセットも試作イベントから抽出されている
            '→後続処理ではBukaCodeを指定したロジックがあるため、アンマッチが生じる
            '→BlockNo/BukaCodeがEBOMから抽出されるべきセットの場合、EBOMのみからの部品抽出ロジックに回す
            '→試作イベントベース車はブロック抽出時にBukaCodeを最新に置き換えている
            '→この時点でBukaCodeが最新でないのは、EBOMベース車
            Dim SekkeikaDao As TShisakuBlockSekkeikaTmpDao = New TShisakuBlockSekkeikaTmpDaoImpl
            Dim SekkeikaVo As TShisakuBlockSekkeikaTmpVo = SekkeikaDao.FindByPk(blockVo.ShisakuEventCode, blockVo.ShisakuBlockNo)
            If SekkeikaVo Is Nothing Then
                _koseiMatrix = New BuhinKoseiMatrix
                Flag = False
            End If
            ...
            If Not Flag Then
                ...
                Else
                    _koseiMatrix = NewMatrixKoseiTenkai(True, JikyuUmu, False, yakanFlg)
                End If
            Else
                Flag = True
            End If
            ...
            If Not _koseiMatrix Is Nothing Then
                ...
                If _koseiMatrix.Records.Count = 0 Then
                    '最初のINSTL品番列が無ければ構成は無い・・・はず'・・・なわけない、eBom側にあるかもしれないよ
                    'If Not Flag Then
                    '    DeleteBySekkeiBlockAndInstl()
                    'End If

                    ''2015/10/02 追加 E.Ubukata Ver.2.11.5
                    Dim impl As BuhinEditBaseDao = New BuhinEditBaseDaoImpl
                    If Flag Then
                        Dim MaxUpdateBuhinNoHyoujijun As Integer = 0
                        Dim MaxUpdateInstlHyoujijun As Integer = 0
                        Dim _koseiMatrix4ebom = New BuhinKoseiMatrix
                        If Not yakanFlg Then
                            _koseiMatrix4ebom = NewMatrixKoseiTenkai(True, JikyuUmu, True)
                        Else
                            _koseiMatrix4ebom = NewMatrixKoseiTenkai(True, JikyuUmu, True, yakanFlg)
                        End If
                        If _koseiMatrix4ebom.Records.Count > 0 Then
                            UpdateBySekkeiBlockInstlAndBaseEbom(_koseiMatrix4ebom, MaxUpdateInstlHyoujijun, instlTitle)
                            InsertByBuhinEditAndBase(_koseiMatrix4ebom, impl, MaxUpdateBuhinNoHyoujijun)
                            InsertByBuhinEditInstlAndBase(_koseiMatrix4ebom, impl, MaxUpdateBuhinNoHyoujijun, MaxUpdateInstlHyoujijun, instlTitle)
                        End If
                    Else
                        '量産型E-BOMはこっち'
                        InsertByBuhinEditAndBase(_koseiMatrix, impl)
                        InsertByBuhinEditInstlAndBase(_koseiMatrix, impl)
                ...
                Else
                    ...
                    If Flag Then
                        ...
                        Dim MaxUpdateBuhinNoHyoujijun As Integer = 0
                        InsertByBuhinEditAndBaseEvent(_koseiMatrix, impl, MaxUpdateBuhinNoHyoujijun)
                        InsertByBuhinEditInstlAndBaseEvent(_koseiMatrix, impl, instlTitle)
                        Dim MaxUpdateInstlHyoujijun As Integer = 0
                        UpdateBySekkeiBlockInstlAndBase(_koseiMatrix, impl, MaxUpdateInstlHyoujijun, instlTitle)

                        'EBOM、イベント組み合わせ設定時の、EBOM部品構成追加処理
                        'ベース車にイベントを指定した場合、INSTL表示順を0から振りなおしている。
                        '影響考慮して、EBOM部品情報はイベント部品情報の後に付加する。
                        Dim _koseiMatrix4ebom = New BuhinKoseiMatrix
                        If Not yakanFlg Then
                            _koseiMatrix4ebom = NewMatrixKoseiTenkai(True, JikyuUmu, True)
                        Else
...
#End Region
        ...
        ''' <returns>新しい部品構成編集クラス</returns>
        ''' <remarks></remarks>
        Public Function NewMatrixKoseiTenkai(ByVal isReuseStructure As Boolean, ByVal JikyuUmu As String, Optional ByVal flg As Boolean = False, Optional ByVal yakanFlg As Boolean = False) As BuhinKoseiMatrix
            '↓↓2014/09/25 酒井 ADD BEGIN
            '        Public Function NewMatrixKoseiTenkai(ByVal isReuseStructure As Boolean, ByVal JikyuUmu As String, Optional ByVal flg As Boolean = False) As BuhinKoseiMatrix
            ...
            ''↓↓2014/09/15 1 ベース部品表作成表機能増強 酒井ADD BEGIN
            'Dim indexes As ICollection(Of Integer) = MergeCollectionAsSorted(_koseiMatrix.GetInputInsuColumnIndexes(), instlTitle.GetInputInstlHinbanColumnIndexes)
            Dim indexes As ICollection(Of Integer)
            If Not flg Then
                indexes = MergeCollectionAsSorted(_koseiMatrix.GetInputInsuColumnIndexes(), instlTitle.GetInputInstlHinbanColumnIndexes)
            Else
                'Ebom/イベント組み合わせ時、_koseiMatrixにはベース車イベントの部品情報が入っままのため、newMatrixを使う。
                indexes = MergeCollectionAsSorted(newMatrix.GetInputInsuColumnIndexes(), instlTitle.GetInputInstlHinbanColumnIndexes)
            End If
            ''↑↑2014/09/15 1 ベース部品表作成表機能増強 酒井ADD END
            ...
            For Each columnIndex As Integer In indexes
                ...
                'EBOM/イベント組み合わせ抽出時、イベントベース車INSTLの部品情報をダミーで作成する。
                If instlTitle.InstlDataKbn(columnIndex) = "0" Or StringUtil.IsEmpty(instlTitle.InstlDataKbn(columnIndex)) Then
                    ...
                    If GetStructureResult(columnIndex).IsExist Then
                        If GetStructureResult(columnIndex).IsEBom Then
                            ...
                            Dim columnBag As New InstlColumnBag
                            If Not yakanFlg Then
                                columnBag = CreateInstlColumnBag(GetStructureResult(columnIndex))
                            Else
                                columnBag = CreateInstlColumnBag(GetStructureResult(columnIndex), yakanFlg)
                            End If
                            '↑↑2014/09/25 酒井 ADD END
                            '--------------------------------------------------------------------------------------
                            '２次改修
                ...
                Else
                    ...
                    'BuhinNo=Nothingにすることで、BuhinEditBaseDaoImpl.InsertBySekkeiBuhinEditでダミー行を読み飛ばす。
                    RVo.BuhinNo = Nothing
                    ...
                    'insu=Nothingにすることで、BuhinEditBaseDaoImpl.InsertBySekkeiBuhinEditInstlでダミー行を読み飛ばす。
                    insuVo.InsuSuryo = Nothing

                    newMatrix.InstlColumn(columnIndex).Add(RVo, insuVo)
                    mergeColumn.Compute(newMatrix.InstlColumn(columnIndex), columnIndex)
                End If
                ''↑↑2014/09/12 1 ベース部品表作成表機能増強 酒井ADD END
            Next
            Return newMatrix
        ...
        ''' <remarks></remarks>
        Private Function CreateInstlColumnBag(ByVal aStructureResult As StructureResult, Optional ByVal yakanFlg As Boolean = False) As InstlColumnBag
            Dim koseimatrix As New BuhinKoseiMatrix
            If Not yakanFlg Then
                koseimatrix = GetNewKoseiMatrix(aStructureResult)
            Else
                koseimatrix = GetNewKoseiMatrix(aStructureResult, yakanFlg)
            End If
        ...
        Private Function GetNewKoseiMatrix(ByVal aStructureResult As StructureResult, Optional ByVal yakanFlg As Boolean = False) As BuhinKoseiMatrix
            If Not yakanFlg Then
                Return GetNewKoseiMatrix(aStructureResult, 0, JikyuUmu)
            Else
                Return GetNewKoseiMatrix(aStructureResult, 0, JikyuUmu, yakanFlg)
            End If
        End Function
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Private Function GetNewKoseiMatrix(ByVal aStructureResult As StructureResult, ByVal baseLevel As Integer?, ByVal JikyuUmu As String, Optional ByVal yakanFlg As Boolean = False) As BuhinKoseiMatrix
            If Not yakanFlg Then
                Return make.Compute(aStructureResult, 0, baseLevel)
            Else
                Return make.Compute(aStructureResult, 0, baseLevel, "", "", "", False, yakanFlg)
            End If
        End Function
...
#Region "部品編集情報を登録する"
        ...
        ''' </summary>
        ''' <param name="koseiMatrix">構成マトリクス(部品表編集情報)</param>
        ''' <param name="impl">部品表編集ベースDao</param>
        ''' <remarks></remarks>
        Private Sub InsertByBuhinEditAndBase(ByVal koseiMatrix As BuhinKoseiMatrix, ByVal impl As BuhinEditBaseDao, Optional ByVal MaxUpdateBuhinNoHyoujijun As Integer = 0)
            impl.InsertBySekkeiBuhinEdit(blockVo.ShisakuEventCode, _
                             ...
                             login, _
                             aShisakuDate, MaxUpdateBuhinNoHyoujijun)
        End Sub
        ...
        ''' <param name="impl">部品表編集ベースDao</param>
        ''' <remarks></remarks>
        Private Sub InsertByBuhinEditInstlAndBase(ByVal koseiMatrix As BuhinKoseiMatrix, ByVal impl As BuhinEditBaseDao, Optional ByVal MaxUpdateBuhinNoHyoujijun As Integer = 0, Optional ByVal MaxUpdateInstlHyoujijun As Integer = 0, Optional ByVal instlTitle As BuhinEditKoseiInstlTitle = Nothing)
            impl.InsertBySekkeiBuhinEditInstl(blockVo.ShisakuEventCode, _
                                 ...
                                 koseiMatrix, _
                                 login, _
                                 aShisakuDate, MaxUpdateBuhinNoHyoujijun, MaxUpdateInstlHyoujijun, instlTitle)
        End Sub
        ...
        ''' <param name="impl">部品表編集ベースDao</param>
        ''' <remarks></remarks>
        Private Sub InsertByBuhinEditAndBaseEvent(ByVal koseiMatrix As BuhinKoseiMatrix, ByVal impl As BuhinEditBaseDao, Optional ByRef MaxUpdateBuhinNoHyoujijun As Integer = 0)
        ...
        ''' <param name="impl">部品表編集ベースDao</param>
        ''' <remarks></remarks>
        Private Sub InsertByBuhinEditInstlAndBaseEvent(ByVal koseiMatrix As BuhinKoseiMatrix, ByVal impl As BuhinEditBaseDao, Optional ByVal paraInstlTitle As BuhinEditKoseiInstlTitle = Nothing)
        ...
        ''' <param name="impl">部品表編集ベースDao</param>
        ''' <remarks></remarks>
        Private Sub UpdateBySekkeiBlockInstlAndBase(ByVal koseiMatrix As BuhinKoseiMatrix, ByVal impl As BuhinEditBaseDao, Optional ByRef MaxUpdateInstlHyoujijun As Integer = 0, Optional ByVal paraInstlTitle As BuhinEditKoseiInstlTitle = Nothing)
            impl.UpdateBySekkeiBlockInstl(blockVo.ShisakuEventCode, _
                                             blockVo.ShisakuBukaCode, _
                                             blockVo.ShisakuBlockNo, _
                                             koseiMatrix, MaxUpdateInstlHyoujijun, paraInstlTitle)
        End Sub

        Private Sub UpdateByBuhinEditInstl(ByVal impl As BuhinEditBaseDao)

            impl.UpdateNewInstlHyoujiJun2(blockVo)


        End Sub

        Private Sub UpdateBySekkeiBlockInstlAndBaseEbom(ByVal koseiMatrix As BuhinKoseiMatrix, Optional ByRef MaxUpdateInstlHyoujijun As Integer = 0, Optional ByVal paraInstlTitle As BuhinEditKoseiInstlTitle = Nothing)
            Dim impl As BuhinEditBaseDao = New BuhinEditBaseDaoImpl
            impl.UpdateBySekkeiBlockInstlEbom(blockVo.ShisakuEventCode, _
                                             blockVo.ShisakuBukaCode, _
                                             blockVo.ShisakuBlockNo, _
                                             koseiMatrix, MaxUpdateInstlHyoujijun, paraInstlTitle)
        End Sub
